...
void help()
{
	cout << "Call: ./ch2_ex2_8 faceScene.jpg" << endl;
}
int main( int argc, char** argv )
{
  ...
  Mat img_rgb = imread( argv[1], -1 );
	...
	if(img_rgb.empty())
	{
	  ...
	  cvtColor(img_rgb, out ,CV_BGR2GRAY);
  imshow("Example Gray", out );
  pyrDown(out,out);
  pyrDown(out,out);

  imshow("Example Pyr", out );
  Canny(out,out,10,100,3,true);

  imshow("Example Canny", out );
