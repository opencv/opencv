...
void mexFunction(int nlhs, mxArray* plhs[],
  ...
  // parse the inputs
  ArgumentParser parser("pyrMeanShiftFiltering");
  parser.addVariant("pyrMeanShiftFiltering", 3, 2, "maxLevel", "termcrit");
  ...
  double sp = inputs[1].toDouble();
  double sr = inputs[2].toDouble();
  int maxLevel = inputs[3].empty() ? (int) 1 : inputs[3].toInt();
  TermCriteria termcrit = inputs[4].empty() ? (TermCriteria) TermCriteria(TermCriteria::MAX_ITER+TermCriteria::EPS,5,1) : inputs[4].toTermCriteria();
  Mat dst;
  ...
  try {
    ...
    error("Uncaught exception occurred in pyrMeanShiftFiltering");
  }
