...
void mexFunction(int nlhs, mxArray* plhs[],
  ...
  // parse the inputs
  ArgumentParser parser("Scharr");
  parser.addVariant("Scharr", 4, 3, "scale", "delta", "borderType");
  ...
  int dx = inputs[2].toInt();
  int dy = inputs[3].toInt();
  double scale = inputs[4].empty() ? (double) 1 : inputs[4].toDouble();
  double delta = inputs[5].empty() ? (double) 0 : inputs[5].toDouble();
  int borderType = inputs[6].empty() ? (int) BORDER_DEFAULT : inputs[6].toInt();
  Mat dst;
  ...
  try {
    ...
    error("Uncaught exception occurred in Scharr");
  }
