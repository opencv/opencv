...
void mexFunction(int nlhs, mxArray* plhs[],
  ...
  // parse the inputs
  ArgumentParser parser("fastNlMeansDenoisingColored");
  parser.addVariant("fastNlMeansDenoisingColored", 1, 4, "h", "hColor", "templateWindowSize", "searchWindowSize");
  ...
  float h = inputs[1].empty() ? (float) 3 : inputs[1].toFloat();
  float hColor = inputs[2].empty() ? (float) 3 : inputs[2].toFloat();
  int templateWindowSize = inputs[3].empty() ? (int) 7 : inputs[3].toInt();
  int searchWindowSize = inputs[4].empty() ? (int) 21 : inputs[4].toInt();
  Mat dst;
  ...
  try {
    ...
    error("Uncaught exception occurred in fastNlMeansDenoisingColored");
  }
