...
void mexFunction(int nlhs, mxArray* plhs[],
  ...
  // parse the inputs
  ArgumentParser parser("fastNlMeansDenoising");
  parser.addVariant("fastNlMeansDenoising", 2, 3, "templateWindowSize", "searchWindowSize", "normType");
  ...
  vector_float h = inputs[1].toVectorFloat();
  int templateWindowSize = inputs[2].empty() ? (int) 7 : inputs[2].toInt();
  int searchWindowSize = inputs[3].empty() ? (int) 21 : inputs[3].toInt();
  int normType = inputs[4].empty() ? (int) NORM_L2 : inputs[4].toInt();
  Mat dst;
  ...
  try {
    ...
    error("Uncaught exception occurred in fastNlMeansDenoising");
  }
