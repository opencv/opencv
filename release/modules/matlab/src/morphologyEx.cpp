...
void mexFunction(int nlhs, mxArray* plhs[],
  ...
  // parse the inputs
  ArgumentParser parser("morphologyEx");
  parser.addVariant("morphologyEx", 3, 4, "anchor", "iterations", "borderType", "borderValue");
  ...
  int op = inputs[1].toInt();
  Mat kernel = inputs[2].toMat();
  Point anchor = inputs[3].empty() ? (Point) Point(-1,-1) : inputs[3].toPoint();
  int iterations = inputs[4].empty() ? (int) 1 : inputs[4].toInt();
  int borderType = inputs[5].empty() ? (int) BORDER_CONSTANT : inputs[5].toInt();
  Scalar borderValue = inputs[6].empty() ? (Scalar) morphologyDefaultBorderValue() : inputs[6].toScalar();
  Mat dst;
  ...
  try {
    ...
    error("Uncaught exception occurred in morphologyEx");
  }
