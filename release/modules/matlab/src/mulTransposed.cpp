...
void mexFunction(int nlhs, mxArray* plhs[],
  ...
  // parse the inputs
  ArgumentParser parser("mulTransposed");
  parser.addVariant("mulTransposed", 2, 3, "delta", "scale", "dtype");
  ...
  bool aTa = inputs[1].toBool();
  Mat delta = inputs[2].empty() ? (Mat) Mat() : inputs[2].toMat();
  double scale = inputs[3].empty() ? (double) 1 : inputs[3].toDouble();
  int dtype = inputs[4].empty() ? (int) -1 : inputs[4].toInt();
  Mat dst;
  ...
  try {
    ...
    error("Uncaught exception occurred in mulTransposed");
  }
