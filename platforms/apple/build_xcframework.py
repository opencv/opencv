#!/usr/bin/env python3
"""
This script builds OpenCV into an xcframework for iOS device and simulator targets.
"""
...
if __name__ == "__main__":
    ...
    description = """
        This script builds OpenCV into an xcframework for iOS device and simulator.
        """
    epilog = """
        Any arguments that are not recognized by this script are passed through to platforms/ios/build_framework.py.
        """
    ...
    parser.add_argument('--framework_name', default='opencv2', help='Name of OpenCV xcframework (default: opencv2)')
    ...
    if unknown_args:
        print("The following args are not recognized by this script and will be passed through to build_framework.py: {}".format(unknown_args))
    ...
    try:
        ios_script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'ios', 'build_framework.py'))
        build_folders = []
        ...
        if iphoneos_archs:
            ...
            command = [sys.executable, ios_script_path, build_folder, "--iphoneos_archs", iphoneos_archs,
                       "--framework_name", args.framework_name, "--build_only_specified_archs"] + unknown_args
        ...
        if iphonesimulator_archs:
            ...
            build_folders.append(build_folder)
            command = [sys.executable, ios_script_path, build_folder, "--iphonesimulator_archs", iphonesimulator_archs,
                       "--framework_name", args.framework_name, "--build_only_specified_archs"] + unknown_args
        ...
        for folder in build_folders:
            xcframework_build_command += ["-framework", os.path.join(folder, "%s.framework" % args.framework_name)]
