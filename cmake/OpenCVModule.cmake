...
")
    ...
    set(OPENCV_MODULE_${the_module}_PACKAGE "${the_package}" CACHE INTERNAL "")
...
macro(_glob_modules_locations out_paths out_names)
  set(__PATHS ${ARGN})
  set(__paths "")
  set(__names "")
  foreach(path ${__PATHS})
    #message(STATUS "Inspect: ${path}")
    list(LENGTH __paths __before)
    get_filename_component(path "${path}" ABSOLUTE)
    # Either module itself
    if(NOT path STREQUAL CMAKE_CURRENT_SOURCE_DIR AND EXISTS "${path}/CMakeLists.txt")
      get_filename_component(__name "${path}" NAME)
      list(APPEND __paths "${path}")
      list(APPEND __names "${__name}")
    else()
      if(EXISTS "${path}/opencv_package.cmake")
        include("${path}/opencv_package.cmake")
      endif()
      if(DEFINED OPENCV_PACKAGE_NAME)
        list(APPEND OPENCV_PACKAGES "${OPENCV_PACKAGE_NAME}")
        set(OPENCV_PACKAGE_${OPENCV_PACKAGE_NAME}_LOCATION "${path}")
      endif()
      # Either flat collection of modules
      file(GLOB __subdirs RELATIVE "${path}" "${path}/*")
      foreach(subdir ${__subdirs})
        #message(STATUS "Inspect: ${path}/${subdir}")
        if(EXISTS "${path}/${subdir}/CMakeLists.txt")
          list(APPEND __paths "${path}/${subdir}")
          list(APPEND __names "${subdir}")
        endif()
      endforeach()
    endif()
    list(LENGTH __paths __after)
    if(__before EQUAL __after)
      message(SEND_ERROR "No modules has been found: ${path}")
    endif()
  endforeach()
  # Return
  set(${out_paths} ${__paths})
  set(${out_names} ${__names})
  unset(__PATHS)
  unset(__paths)
  unset(__names)
endmacro()
...
macro(_add_modules_1 paths names)
  ...
  if(NOT __len EQUAL 0)
    ...
    foreach(i RANGE ${__len})
      ...
      # specify module package (main, contrib, etc)
      set(the_package "")
      ocv_assert(OPENCV_PACKAGES)
      foreach(package ${OPENCV_PACKAGES})
        ocv_is_subdir(res "${OPENCV_PACKAGE_${package}_LOCATION}" "${__path}")
        if(res OR "${OPENCV_PACKAGE_${package}_LOCATION}" STREQUAL "${__path}")
          set(the_package "${package}")
          break()
        endif()
      endforeach()
      ocv_assert(the_package)
...
macro(_add_modules_2)
  ...
  foreach(m ${ARGN})
    ...
    if(BUILD_opencv_world AND m STREQUAL "opencv_world"
      ...
      set(the_package "${OPENCV_MODULE_${m}_PACKAGE}")
      string(TOUPPER "${the_package}" the_package_upper)
      list(APPEND OPENCV_PACKAGE_${the_package_upper}_MODULES ${m})
...
macro(ocv_glob_modules main_root)
  ...
  set(OPENCV_PACKAGE_NAME "main")
  _glob_modules_locations(__main_paths __main_names ${main_root})
  set(OPENCV_PACKAGE_NAME "extra")
  _glob_modules_locations(__extra_paths __extra_names ${ARGN})
  set(OPENCV_PACKAGES "${OPENCV_PACKAGES}" CACHE INTERNAL "")
  ...
  if(";${OPENCV_PACKAGES};" MATCHES ";contrib;")
    add_definitions(-DOPENCV_PACKAGE_CONTRIB=1)
  endif()
  ...
  foreach(package ${OPENCV_PACKAGES})
    ...
    ocv_list_sort(OPENCV_PACKAGE_${package_upper}_MODULES)
    set(OPENCV_PACKAGE_${package_upper}_MODULES "${OPENCV_PACKAGE_${package_upper}_MODULES}" PARENT_SCOPE)
  endforeach()
endmacro()
