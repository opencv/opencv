...
function(_glob_locations out_paths out_names)
  ...
  foreach(path ${PATHS})
    ...
    if(NOT path STREQUAL "${OpenCV_SOURCE_DIR}/modules" AND EXISTS "${path}/CMakeLists.txt")
      ...
      # Escape glob special characters in path to handle directories with brackets, etc.
      string(REPLACE "[" "\\[" escaped_path "${path}")
      string(REPLACE "]" "\\]" escaped_path "${escaped_path}")
      string(REPLACE "?" "\\?" escaped_path "${escaped_path}")
      string(REPLACE "*" "\\*" escaped_path "${escaped_path}")
      file(GLOB subdirs RELATIVE "${path}" "${escaped_path}/*")
    ...
    if(before EQUAL after)
      message(SEND_ERROR "No modules have been found: ${path}")
    endif()
  endforeach()
  # Return
