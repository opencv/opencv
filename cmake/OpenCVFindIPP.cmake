...
macro(ipp_detect_version)
  ...
  macro(_ipp_add_library name)
    ...
    set(IPP_LIBRARYFILEPART ${IPP_LIB_PREFIX}${IPP_PREFIX}${name}${IPP_SUFFIX}${IPP_LIB_SUFFIX})
    if (MINGW AND NOT EXISTS ${IPP_LIBRARY_DIR}/${IPP_LIBRARYFILEPART})
      # IPP is named ippicvmt.lib
      set(IPP_LIBRARYFILEPART ippicvmt.lib)
    endif()

    if (EXISTS ${IPP_LIBRARY_DIR}/${IPP_LIBRARYFILEPART})
      if (BUILD_WITH_DYNAMIC_IPP AND NOT HAVE_IPP_ICV)
        # When using dynamic libraries from standalone Intel IPP it is your responsibility to install those on the target system
        list(APPEND IPP_LIBRARIES ${IPP_LIBRARY_DIR}/${IPP_LIBRARYFILEPART})
      else ()
        add_library(ipp${name} STATIC IMPORTED)
        set_target_properties(ipp${name} PROPERTIES
          IMPORTED_LINK_INTERFACE_LIBRARIES ""
          IMPORTED_LOCATION ${IPP_LIBRARY_DIR}/${IPP_LIBRARYFILEPART}
        )
        ...
        if (NOT BUILD_SHARED_LIBS)
          ...
          install(FILES ${IPP_LIBRARY_DIR}/${IPP_LIBRARYFILEPART}
                  DESTINATION ${OPENCV_3P_LIB_INSTALL_PATH} COMPONENT dev)
          string(TOUPPER ${name} uname)
          set(IPP${uname}_INSTALL_PATH "${CMAKE_INSTALL_PREFIX}/${OPENCV_3P_LIB_INSTALL_PATH}/${IPP_LIBRARYFILEPART}" CACHE INTERNAL "" FORCE)
          set(IPP${uname}_LOCATION_PATH "${IPP_LIBRARY_DIR}/${IPP_LIBRARYFILEPART}" CACHE INTERNAL "" FORCE)
        endif()
      endif()
    else()
      message(STATUS "Can't find Intel IPP library: ${name} at ${IPP_LIBRARY_DIR}/${IPP_LIBRARYFILEPART}")
    endif()
  endmacro()
...
if(WIN32 AND MINGW AND NOT IPP_VERSION_MAJOR LESS 7)
    ...
    # New version with custom implementation (e.g. for mingw32 mxe)
    add_library(RunTmChkAlt ${OpenCV_SOURCE_DIR}/3rdparty/ippicv/runtmchk_mingw.c)
    set(MSV_RUNTMCHK "RunTmChkAlt")
