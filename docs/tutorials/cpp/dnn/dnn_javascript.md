...
```{doxysnippet} dnn/js_face_recognition.html
:tag: Run face detection model
:language: cpp
```
...
1. Run face recognition network to receive `128`-dimensional unit feature vector by input face image.

   ```{doxysnippet} dnn/js_face_recognition.html
   :tag: Get 128 floating points feature vector
   :language: cpp
   ```

1. Perform a recognition.
