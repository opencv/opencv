...
If you want to use Eclipse head to [Using OpenCV Java with Eclipse](java_eclipse.md). For further reading after this guide, look at
the [Introduction into Android Development](android_dev_intro.md) tutorials.
...
:::{note}
Windows users can find the prebuilt files needed for Java development in the
...
:::{note}
When OpenCV is built as a set of **static** libraries (-DBUILD_SHARED_LIBS=OFF option) the
...
:::{note}
The described sample is provided with OpenCV library in the `opencv/samples/java/ant`
folder.
:::
...
-   In this folder create the `build.xml` file with the following content using any text editor:
    ...
    ```{doxyinclude} samples/java/ant/build.xml
    :language: xml
    ```
