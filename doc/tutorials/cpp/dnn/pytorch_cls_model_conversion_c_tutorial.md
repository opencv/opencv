(pytorch_cls_c_tutorial_dnn_conversion)=
...
## Introduction
Let's briefly view the key concepts involved in the pipeline of PyTorch models transition with OpenCV API. The initial step in conversion of PyTorch models into [cv::dnn::Net](https://docs.opencv.org/5.x/db/d30/classcv_1_1dnn_1_1Net.html)
...
Further the obtained ``.onnx`` model is passed into [cv::dnn::readNetFromONNX](https://docs.opencv.org/5.x/d6/d0f/group__dnn.html#ga31e39be3f7e431531c3dc7be2d67c0bf) or [cv::dnn::readNet](https://docs.opencv.org/5.x/d6/d0f/group__dnn.html#ga9fd52c20cdbdd3eb397d8cea8983b63e).
...
For OpenCV-Python building from source, follow the corresponding instructions from the [Introduction to OpenCV](https://docs.opencv.org/5.x/da/df6/tutorial_py_table_of_contents_setup.html).
...
```{figure} images/squirrel_cls.jpg
:alt: Classification model input image
...
1. read the model with [cv::dnn::readNet](https://docs.opencv.org/5.x/d6/d0f/group__dnn.html#ga9fd52c20cdbdd3eb397d8cea8983b63e), initialize the network:
...
In this step we use [cv::dnn::blobFromImage](https://docs.opencv.org/5.x/d6/d0f/group__dnn.html#ga29f34df9376379a603acd8df581ac8d7) function to prepare model input.
...
It should be noted that firstly in [cv::dnn::blobFromImage](https://docs.opencv.org/5.x/d6/d0f/group__dnn.html#ga29f34df9376379a603acd8df581ac8d7) mean value is subtracted and only then pixel values are multiplied by scale.
...
```{figure} images/opencv_resnet50_test_res_c.jpg
:alt: ResNet50 OpenCV C++ inference output
