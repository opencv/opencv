...
The full tutorial code is [here](https://github.com/opencv/opencv/blob/5.x/samples/cpp/tutorial_code/core/how_to_use_OpenCV_parallel_for_/how_to_use_OpenCV_parallel_for_new.cpp).
...
```{figure} images/convolution-example-matrix.gif
...
```{doxysnippet} how_to_use_OpenCV_parallel_for_new.cpp
:tag: convolution-sequential
:language: cpp
```
...
```{doxysnippet} how_to_use_OpenCV_parallel_for_new.cpp
:tag: convolution-kernel-loop
:language: cpp
```

#### Parallel implementation

When looking at the sequential implementation, we can notice that each pixel depends on multiple neighbouring pixels but only one pixel is edited at a time. Thus, to optimize the computation, we can split the image into stripes and parallelly perform convolution on each, by exploiting the multi-core architecture of modern processor. The OpenCV [cv::parallel_for_](https://docs.opencv.org/5.x/d3/d89/group__core__parallel.html#gaa42ec9937b847cb52a97c613fc894c4a) framework automatically decides how to split the computation efficiently and does most of the work for us.
...
:::
We first declare a custom class that inherits from [cv::ParallelLoopBody](https://docs.opencv.org/5.x/d2/d74/classcv_1_1ParallelLoopBody.html) and override the `virtual void operator ()(const cv::Range& range) const`.

```{doxysnippet} how_to_use_OpenCV_parallel_for_new.cpp
:tag: convolution-parallel
:language: cpp
```
...
1. Split the entire traversal of the image and obtain the [row, col] coordinate in the following way (as shown in the above code):
   ...
   ```{doxysnippet} how_to_use_OpenCV_parallel_for_new.cpp
   :tag: convolution-parallel-function
   :language: cpp
   ```
...
```{doxysnippet} how_to_use_OpenCV_parallel_for_new.cpp
:tag: convolution-parallel-function-row
:language: cpp
```
...
:::
To set the number of threads, you can use: [cv::setNumThreads](https://docs.opencv.org/5.x/db/de0/group__core__utils.html#gae78625c3c2aa9e0b83ed31b73c6549c0). You can also specify the number of splitting using the nstripes parameter in [cv::parallel_for_](https://docs.opencv.org/5.x/d3/d89/group__core__parallel.html#gaa42ec9937b847cb52a97c613fc894c4a). For instance, if your processor has 4 threads, setting [cv::setNumThreads(2)](https://docs.opencv.org/5.x/db/de0/group__core__utils.html#gae78625c3c2aa9e0b83ed31b73c6549c0) or setting `nstripes=2` should be the same as by default it will use all the processor threads available but will split the workload only on two threads.
...
:::
```{doxysnippet} how_to_use_OpenCV_parallel_for_new.cpp
:tag: convolution-parallel-cxx11
:language: cpp
```
