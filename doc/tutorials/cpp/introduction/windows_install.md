...
1. Choose a build you want to use and download it.
1. Make sure you have admin rights. Unpack the self-extracting archive.
1. You can check the installation at the chosen path as you can see below.
...
1. You must download [cmake (version >=3.9.1)](https://cmake.org) and install it. You must add cmake to PATH variable during installation

1. You must install [git-bash (version>=2.14.1)](https://git-for-windows.github.io/). Don't add git to PATH variable during installation

1. Run git-bash. You observe a command line window.
Suppose you want to build opencv and opencv_contrib in c:/lib

1. In git command line enter following command (if folder does not exist) :
...
1. save this script with name installOCV.sh in c:/lib
...
1. In git command line enter following command :

   ```bash
   ./installOCV.sh
   ```

1. Drink a coffee or two... opencv is ready : That's all!
1. Next time you run this script, opencv and opencv_contrib will be updated and rebuild

(tutorial_windows_install_build)=
...
<div class="responsive-iframe" style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;max-width:100%;margin:1.5rem 0;">
  <iframe style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;" src="https://www.youtube-nocookie.com/embed/NnovZ1cTlMs?rel=0" title="YouTube video" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</div>
```

```{raw} html
<div class="responsive-iframe" style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;max-width:100%;margin:1.5rem 0;">
  <iframe style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;" src="https://www.youtube-nocookie.com/embed/qGNWMcfWwPU?rel=0" title="YouTube video" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</div>
```
...
1. Install [CMake](http://www.cmake.org/cmake/resources/software.html). Simply follow the wizard, no need to add it to the path. The default install
options are OK.
1. Download and install an up-to-date version of msysgit from its [official
...
1. Install [TortoiseGit](http://code.google.com/p/tortoisegit/wiki/Download). Choose the 32 or 64 bit version according to the type of OS you work in.
...
1. Choose a directory in your file system, where you will download the OpenCV libraries to. I
...
1. In this section, I will cover installing the 3rd party libraries.
    1. Download the [Python libraries](http://www.python.org/downloads/) and install it with the default options. You will need a
...
again.
    ...
    1. The easiest way to install Numpy is to just download its binaries from the [sourceforge page](http://sourceforge.net/projects/numpy/files/NumPy/).
Make sure your download and install
exactly the binary for your python version (so for version `2.7`).

    1. For the [Intel Threading Building Blocks (*TBB*)](http://threadingbuildingblocks.org/file.php?fid=77)
...
Now you can continue reading the tutorials with the [How to build applications with OpenCV inside the "Microsoft Visual Studio"](windows_visual_studio_opencv.md) section.
