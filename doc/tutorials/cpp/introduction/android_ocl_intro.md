...
If you need help with anything of the above, you may refer to our [Introduction into Android Development](android_dev_intro.md) guide.
...
```{doxysnippet} samples/android/tutorial-4-opencl/src/org/opencv/samples/tutorial4/MyGLSurfaceView.java
:tag: minimal_surface_view
:language: java
```

:::{note}
we use two renderer classes: one for legacy [Camera](http://developer.android.com/reference/android/hardware/Camera.html) API
...
```{doxysnippet} samples/android/tutorial-4-opencl/src/org/opencv/samples/tutorial4/NativePart.java
:tag: native_part
:language: java
```
...
```{doxysnippet} samples/android/tutorial-4-opencl/jni/CLprocessor.cpp
:tag: init_opencl
:language: cpp
```
...
All that you need is to pass the created OpenCL context to OpenCV (via [cv::ocl::attachContext()](https://docs.opencv.org/5.x/dc/d83/group__core__opencl.html#gaf83af36be2097fd43e3eba5c8f3025b1)) and somehow wrap OpenGL texture with [cv::UMat](https://docs.opencv.org/5.x/d7/d45/classcv_1_1UMat.html).
...
To compare the performance we measured FPS of the same preview frames modification (_Laplacian_) done by C/C++ code (call to [cv::Laplacian](https://docs.opencv.org/5.x/d4/d86/group__imgproc__filter.html#gad78703e4c8fe703d479c1860d76429e6) with [cv::Mat](https://docs.opencv.org/5.x/d3/d63/classcv_1_1Mat.html)),
by direct OpenCL calls (using OpenCL _images_ for input and output), and by OpenCV _T-API_ (call to [cv::Laplacian](https://docs.opencv.org/5.x/d4/d86/group__imgproc__filter.html#gad78703e4c8fe703d479c1860d76429e6) with [cv::UMat](https://docs.opencv.org/5.x/d7/d45/classcv_1_1UMat.html)) on _Sony Xperia Z3_ with 720p camera resolution:
