...
assumption that you have read and completed with success the [Installation in Windows](windows_install.md) tutorial.
...
[Set the OpenCV environment variable and add it to the systems path](windows_install.md#tutorial-windows-install-path).
...
our source files as described in the [Installation in Windows](windows_install.md) tutorial. When you use this the
...
setting we already made in our previous tutorial [Set the OpenCV environment variable and add it to the systems path](windows_install.md#tutorial-windows-install-path).
...
```{doxyinclude} cpp/tutorial_code/introduction/windows_visual_studio_opencv/introduction_windows_vs.cpp
:language: cpp
```
...
in-depth explanation of the upper source code: [Getting Started with Images](display_image.md).
