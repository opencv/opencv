...
1. Having installed OpenCV. If not yet, go [here](linux_install.md).
...
1. Choose a name for your project (i.e. DisplayImage). An **Empty Project** should be okay for this
example.

![](images/a1.png)

1. Leave everything else by default. Press **Finish**.
1. Your project (in this case DisplayImage) should appear in the **Project Navigator** (usually at
the left side of your window).

![](images/a3.png)

1. Now, let's add a source file using OpenCV:
...
1. So, now you have a project with a empty .cpp file. Let's fill it with some sample code (in other
...
1. We are only missing one final step: To tell OpenCV where the OpenCV headers and libraries are.
...
-   In **C/C++ Build**, click on **Settings**. At the right, choose the **Tool Settings** Tab.
    Here we will enter the headers and libraries info:
        1. In **GCC C++ Compiler**, go to **Includes**. In **Include paths(-l)** you should
...
1. Under C/C++ Application you will see the name of your executable + Debug (if not, click over
C/C++ Application a couple of times). Select the name (in this case **DisplayImage Debug**).
1. Now, in the right side of the window, choose the **Arguments** Tab. Write the path of the image
file we want to open (path relative to the workspace/DisplayImage folder). Let's use
**HappyLittleFish.png**:

![](images/a14.png)

1. Click on the **Apply** button and then in Run. An OpenCV window should pop up with the fish
image (or whatever you used).

![](images/a15.jpg)

1. Congratulations! You are ready to have fun with OpenCV using Eclipse.
...
1. Put a `CmakeLists.txt` file in build:
...
1. Then click configure and then generate. If it's OK, **quit cmake-gui**
1. Run `make -j4` (the -j4 is optional, it just tells the compiler to build in 4 threads). Make
sure it builds.
1. Start eclipse. Put the workspace in some directory but **not** in foo or `foo\build`
1. Right click in the Project Explorer section. Select Import And then open the C/C++ filter.
Choose *Existing Code* as a Makefile Project.
1. Name your project, say *helloworld*. Browse to the Existing Code location `foo\build` (where
you ran your cmake-gui from). Select *Linux GCC* in the *"Toolchain for Indexer Settings"* and
press *Finish*.
1. Right click in the Project Explorer section. Select Properties. Under C/C++ Build, set the
...
`${workspace_loc:/helloworld}/build` since that's where you are building to.
    ...
    1. You can also optionally modify the Build command: from make to something like
