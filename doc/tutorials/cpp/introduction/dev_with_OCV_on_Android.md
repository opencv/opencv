...
8. Before using any OpenCV function you have to load the library first. If you application includes other OpenCV-dependent native libraries you should load them ***after*** OpenCV initialization.
    ...
    ```{doxysnippet} samples/android/tutorial-1-camerapreview/src/org/opencv/samples/tutorial1/Tutorial1Activity.java
    :tag: ocv_loader_init
    :language: java
    ```
...
1. Tell a system that we need camera permissions.
    ...
    ```{doxysnippet} samples/android/tutorial-1-camerapreview/gradle/AndroidManifest.xml
    :tag: camera_permissions
    :language: xml
    ```
...
3. Add camera view to the layout:
    ...
    2. Replace `TextView` with `org.opencv.android.JavaCameraView` widget:
       ...
       ```{doxysnippet} /samples/android/tutorial-1-camerapreview/res/layout/tutorial1_surface_view.xml
       :tag: camera_view
       :language: xml
       ```
   ...
   ```{doxyinclude} /samples/android/tutorial-1-camerapreview/res/layout/tutorial1_surface_view.xml
   :language: xml
   ```
...
```{doxyinclude} samples/android/tutorial-1-camerapreview/src/org/opencv/samples/tutorial1/Tutorial1Activity.java
:language: java
```
...
:::{note}
Do not save or use `CvCameraViewFrame` object out of `onCameraFrame` callback. This object does
