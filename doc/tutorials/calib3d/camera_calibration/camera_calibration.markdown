...
will be $(x_{distorted} y_{distorted})$. The presence of the radial distortion manifests in form
...
unknown parameters are $f_x$ and $f_y$ (camera focal lengths) and $(c_x, c_y)$ which are the optical
centers expressed in pixels coordinates. If for both axes a common focal length is used with a given
$a$ aspect ratio (usually 1), then $f_y=f_x*a$ and in the upper formula we will have a single focal
length $f$. The matrix containing these four parameters is referred to as the *camera matrix*. While
...
on how to do this you can find in the [File Input and Output using XML / YAML / JSON files](../core/file_input_output_with_xml_yml.md) tutorial.
...
We do the calibration with the help of the [cv::calibrateCameraRO](https://docs.opencv.org/5.x/d4/d93/group__calib.html#gaa672b7472ff6f2ee2d0b70d398f73a59) function. It has the following
parameters:
...
-   The object points. This is a vector of *Point3f* vector that for each input image describes how
    ...
    *calcBoardCornerPositions* function as:
    ...
    :::{note}
    If your calibration board is inaccurate, unmeasured, roughly planar targets (Checkerboard
    patterns on paper using off-the-shelf printers are the most convenient calibration targets and
    most of them are not accurate enough.), a method from [\[strobl2011iccv\]](https://docs.opencv.org/5.x/d0/de3/citelist.html#CITEREF_strobl2011iccv) can be utilized to
...
-   The image points. This is a vector of *Point2f* vector which for each input image contains
    ...
    circle pattern). We have already collected this from [cv::findChessboardCorners](https://docs.opencv.org/5.x/d4/d93/group__calib.html#ga93efa9b0aa890de240ca32b11253dd4a) or [cv::findCirclesGrid](https://docs.opencv.org/5.x/d4/d93/group__calib.html#ga7f02cd21c8352142890190227628fa80) function. We just need to pass it on.
...
-   The index of the object point to be fixed. We set it to -1 to request standard calibration method.
    ...
    of the calibration board grid. See [cv::calibrateCameraRO](https://docs.opencv.org/5.x/d4/d93/group__calib.html#gaa672b7472ff6f2ee2d0b70d398f73a59) for detailed explanation.
...
-   The function returns the average re-projection error. This number gives a good estimation of
    ...
    by using the [cv::projectPoints](https://docs.opencv.org/5.x/da/d35/group____3d.html#gaa9baca117e5ab7d588e23b50b2ff4509) to first transform the object point to image point. Then we
...
Let there be [this input chessboard pattern ](images/pattern.png) which has a size of 9 X 6. I've used an
...
The same works for [this asymmetrical circle pattern ](images/acircles_pattern.png) by setting the input
...
Add these values as constants to your program, call the [cv::initUndistortRectifyMap](https://docs.opencv.org/5.x/da/d35/group____3d.html#ga7dfb72c9cf9780a347fbe3d1c47e5d5a) and the
[cv::remap](https://docs.opencv.org/5.x/da/d54/group__imgproc__transform.html#ga399c57f6b292b8883ad8ecbf2581b0dd) function to remove distortion and enjoy distortion free inputs for cheap and low
quality cameras.
...
<div class="responsive-iframe" style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;max-width:100%;margin:1.5rem 0;">
  <iframe style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;" src="https://www.youtube-nocookie.com/embed/ViPN810E0SU?rel=0" title="YouTube video" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</div>
```
