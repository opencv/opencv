...
The `cv::aruco::GridBoard` class is a specialized class that inherits from the `cv::aruco::Board` class and which represents a Board
...
The rejected corners can be obtained from the `cv::aruco::ArucoDetector::detectMarkers()` function and are also known as marker
...
The `cv::aruco::ArucoDetector::refineDetectedMarkers()` function finds correspondences between these candidates and the missing
