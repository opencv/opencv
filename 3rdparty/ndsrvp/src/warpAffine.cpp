...
class NdsrvpWarpAffineInvoker : public ParallelLoopBody {
public:
    NdsrvpWarpAffineInvoker(const Mat& _src, Mat& _dst, int _interpolation, int _borderType,
                ...
                for (y1 = 0; y1 < bh; y1++) {
                    ...
                    if (interpolation == CV_HAL_INTER_NEAREST) {
                        ...
                        for (; x1 < bw; x1 += 2) {
                            ...
                            int32x2_t vx = __nds__v_sclip32(__nds__v_sra32(vX, AB_BITS), 15);
                            int32x2_t vy = __nds__v_sclip32(__nds__v_sra32(vY, AB_BITS), 15);

                            *(uint16x4_t*)(xy + x1 * 2) = __nds__v_pkbb16(*(uint16x4_t*)&vy, *(uint16x4_t*)&vx);
                        }
    ...
    const double M[6], int interpolation, int borderType, const double borderValue[4])
{
    ...
    NdsrvpWarpAffineInvoker invoker(src, dst, interpolation, borderType,
