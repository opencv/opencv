...
// #define cv_hal_split16u cv::cv_hal_rvv::split16u
...
// __attribute__((optimize("no-tree-vectorize")))
// #endif
// inline int split16u(const ushort* src, ushort** dst, int len, int cn) {
//     int k = cn % 4 ? cn % 4 : 4;
//     int i = 0;
...
//             // Векторная обработка
//             for (; i <= len - vl; i += vl) {
//                 auto vec = __riscv_vlse16_v_u16m4(src + i * cn, cn, vl);
//                 __riscv_vse16_v_u16m4(dst0 + i, vec, vl);
//             }
//             // Скалярный остаток
//             #if defined(__clang__)
//             #pragma clang loop vectorize(disable)
...
// __attribute__((optimize("no-tree-vectorize")))
// #endif
// inline int split32s(const int* src, int** dst, int len, int cn) {
//     int k = cn % 4 ? cn % 4 : 4;
//     int i = 0;
//     int vl = __riscv_vsetvlmax_e32m4();
//     if (k == 1) {
//         int* dst0 = dst[0];
//         if (cn == 1) {
//             memcpy(dst0, src, len * sizeof(uchar));
...
//             // Векторная обработка
//             for (; i <= len - vl; i += vl) {
//                 auto vec = __riscv_vlse32_v_i32m4(src + i * cn, cn, vl);
//                 __riscv_vse32_v_i32m4(dst0 + i, vec, vl);
//             }
//             // Скалярный остаток
//             #if defined(__clang__)
//             #pragma clang loop vectorize(disable)
...
// __attribute__((optimize("no-tree-vectorize")))
// #endif
// inline int split64s(const int64* src, int64** dst, int len, int cn) {
//     int k = cn % 4 ? cn % 4 : 4;
//     int i = 0;
//     int vl = __riscv_vsetvlmax_e64m4();
//     if (k == 1) {
//         int64* dst0 = dst[0];
//         if (cn == 1) {
//             memcpy(dst0, src, len * sizeof(uchar));
...
//             // Векторная обработка
//             for (; i <= len - vl; i += vl) {
//                 auto vec = __riscv_vlse64_v_i64m4(src + i * cn, cn, vl);
//                 __riscv_vse64_v_i64m4(dst0 + i, vec, vl);
//             }
//             // Скалярный остаток
//             #if defined(__clang__)
//             #pragma clang loop vectorize(disable)
