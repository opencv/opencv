...
template<> struct rvv<uchar>
{
    ...
    static inline vuint16m8_t vslide1down(vuint16m8_t a, ushort b, size_t c) { return __riscv_vslide1down(a, b, c); }
...
template<> struct rvv<short>
{
    ...
    static inline vint32m8_t vslide1down(vint32m8_t a, int b, size_t c) { return __riscv_vslide1down(a, b, c); }
...
template<> struct rvv<float>
{
    ...
    static inline vfloat32m8_t vslide1down(vfloat32m8_t a, float b, size_t c) { return __riscv_vfslide1down(a, b, c); }
        ...
        if (i + offset_y >= 0 && i + offset_y < full_height)
        {
            ...
            else
            {
                ...
                for (int j = left; j < right; j += vl)
                {
                    ...
                    extra += vl;
                    auto sum = src;
                    src = rvv<T>::vslide1down(src, extra[0], vl);
                    sum = helperWT::vadd(sum, src, vl);
                    src = rvv<T>::vslide1down(src, extra[1], vl);
                    sum = helperWT::vadd(sum, src, vl);
                    if (ksize == 5)
                    {
                        src = rvv<T>::vslide1down(src, extra[2], vl);
                        sum = helperWT::vadd(sum, src, vl);
                        src = rvv<T>::vslide1down(src, extra[3], vl);
                        sum = helperWT::vadd(sum, src, vl);
                    }
                    helperWT::vstore(res.data() + p2idx(i, j), sum, vl);
                }
