...
const float coef1 = 0.9660; 
const float coef2 = -4.3754;
                    ...
                    double scale, bool tryflip)
{
	...
	const int max_neighbors = (float)coef1*smallImg.cols + coef2;
    ...
    for( vector<Rect>::iterator r = faces.begin(); r != faces.end(); r++, i++ )
    {
		...
		const int half_height=(float)(r->height/2);
		r->y=r->y + half_height;
		...
		printf("Detected %d smile neighbors\n", smile_neighbors);
		const int rect_height=(float)img.rows*smile_neighbors/max_neighbors;
