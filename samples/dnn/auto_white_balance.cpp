...
Given an image, the FC4 model predicts scene illuminant (R,G,B). We then apply the illuminant to the image, applying the correction in the linear RGB space.

Yuanming Hu, Baoyuan Wang, and Stephen Lin. “FC⁴: Fully Convolutional Color Constancy with Confidence-Weighted Pooling.” CVPR, 2017, pp. 4085–4094.
*/
...
static void makeInverseMask(const Mat &m, Mat &inv) { bitwise_not(m, inv); }
...
static void annotate(Mat &img, const Vec3f &illum, const string &title) {
  double fs = std::max(0.5, std::min(img.cols, img.rows) / 800.0);
  int th = std::max(1, (int)std::round(fs * 2));
  char buf[256];
  snprintf(buf, sizeof(buf), "Illuminant (R,G,B): %.4f, %.4f, %.4f", illum[0],
           illum[1], illum[2]);
  putText(img, title, Point(10, 30), FONT_HERSHEY_SIMPLEX, fs,
          Scalar(0, 255, 0), th);
  putText(img, buf, Point(10, 60), FONT_HERSHEY_SIMPLEX, fs, Scalar(0, 255, 0),
          th);
}
...
int main(int argc, char **argv) {
  const string about =
      ...
      "Predicts scene illuminant and corrects the white balance of the image.\n\n"
      "Example:\n"
      "\t./auto_white_balance --model=path/to/fc4_fold_0.onnx --input=image.jpg\n";
  ...
  annotate(origVis, illum, "Original");
  annotate(corrVis, illum, "FC4-corrected");
