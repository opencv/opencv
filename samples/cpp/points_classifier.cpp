...
int main()
{
    ...
    for(;;)
    {
        ...
        if( key == 'r' ) // run
        {
...
#ifdef HAVE_OPENCV_OCL
            namedWindow( "kNN_OCL", WINDOW_AUTOSIZE );
            imshow( "kNN_OCL", imgDst );
#else
            ...
            find_decision_boundary_KNN( K );
#ifdef HAVE_OPENCV_OCL
            namedWindow( "kNN2_OCL", WINDOW_AUTOSIZE );
            imshow( "kNN2_OCL", imgDst );
#else
...
#ifdef HAVE_OPENCV_OCL
            namedWindow( "classificationSVM1_OCL", WINDOW_AUTOSIZE );
            imshow( "classificationSVM1_OCL", imgDst );
#else
            ...
            find_decision_boundary_SVM( params );
#ifdef HAVE_OPENCV_OCL
            cvNamedWindow( "classificationSVM2_OCL", WINDOW_AUTOSIZE );
            imshow( "classificationSVM2_OCL", imgDst );
#else
