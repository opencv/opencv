...
class ParallelMeanLine: public ParallelLoopBody
{
private:
    Mat &imgSrc;
...
public:
    ...
    // First mean and variance for each line are computed
        for (int y = range.start; y < range.end; y++)
        {
            ...
            meanLine[y] = m / double(width);
            stdLine[y] = s2/width-meanLine[y]*meanLine[y];
        }
    // We want : all lines must have a mean value of 128 and standard deviation of 64. Negative value or value greater than 255 are threshold
        for (int y = range.start; y < range.end; y++)
        {
            ...
            for (int x = 0; x<width; x++,ptr++)
            {
                   *ptr = saturate_cast<uchar>((*ptr-meanLine[y])*sqrt(64*64/stdLine[y])+128);
            }
        }
...
int main (int argc,char **argv)
{
    ...
    parallel_for_(Range(0,m.rows), x,nbThreads);
