import pdb, sys, traceback, cv2, numpy as np, cv2, os, json, argparse
...
def calibrateFromPoints(pattern_points, image_points, image_sizes, is_fisheye):
	...
	success, Rs, Ts, Ks, distortions, rvecs0, tvecs0, errors_per_frame, output_pairs = cv2.calibrateMultiview(pattern_points_all,
		image_points, image_sizes, visibility, is_fisheye, cv2.USE_INTRINSICS_GUESS=False)
	print(Rs)
	print(Ts)
	print(Ks)
	print(distortions)
...
def calibrateFromImages(files_with_images, grid_size, is_fisheye, dist_m):
	...
	for filename in files_with_images:
		...
		for img_name in images_names:
			...
			if scale < 1.0:
				gray = cv2.resize(gray, (int(scale * gray.shape[1]), int(scale * gray.shape[0])), interpolation=cv2.INTER_AREA)
				window = (16, 16) # increase refinement window

			ret, corners = cv2.findChessboardCorners(gray, grid_size, None)
			if ret:
				if scale < 1.0: corners /= scale
				corners2 = cv2.cornerSubPix(gray, corners, window, (-1,-1), criteria)
