...
DualQuat<T> DualQuat<T>::createFromMat(InputArray _R)
{
    ...
    return createFromQuat(r, trans * r / T(2.0));
}
...
{
    T half_angle = angle / T(2.0), half_d = d / T(2.0);
...
Matx<_Tp, 4, 4> jacob_exp(const Quat<_Tp> &q)
{
    ...
    _Tp sinc_nv = abs(nv) < cv::DualQuat<_Tp>::CV_DUAL_QUAT_EPS ? _Tp(1.0) - nv * nv / _Tp(6.0) : std::sin(nv) / nv;
    _Tp csiii_nv = abs(nv) < cv::DualQuat<_Tp>::CV_DUAL_QUAT_EPS ? -_Tp(1.0) / _Tp(3.0) : (std::cos(nv) - sinc_nv) / nv / nv;
