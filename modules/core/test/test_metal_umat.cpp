...
TEST(Core_Metal_UMat, SharedMemoryUsageMapReadWrite)
{
    if (!cv::metal::haveMetal())
        return;

    Mat src(27, 31, CV_8UC3);
    randu(src, 0, 254);

    UMat u(src.size(), src.type(), USAGE_ALLOCATE_SHARED_MEMORY);
    src.copyTo(u);

    Mat expected = src + Scalar::all(1);
    {
        Mat mapped = u.getMat(ACCESS_RW);
        EXPECT_LE(cvtest::norm(mapped, src, NORM_INF), 0);
        mapped += Scalar::all(1);
    }
...
TEST(Core_Metal_UMat, DeviceMemoryUsageMapRead)
{
    if (!cv::metal::haveMetal())
        return;

    Mat src(23, 29, CV_32FC1);
    randu(src, -5.0f, 5.0f);
    ...
    {
        Mat mapped = u.getMat(ACCESS_READ);
        EXPECT_LE(cvtest::norm(mapped, src, NORM_INF), 0);
    }
...
TEST(Core_Metal_UMat, DeviceMemoryUsageMapWrite)
{
    if (!cv::metal::haveMetal())
        return;

    UMat u(19, 37, CV_8UC4, USAGE_ALLOCATE_DEVICE_MEMORY);
    {
        Mat mapped = u.getMat(ACCESS_WRITE);
        mapped.setTo(Scalar(11, 13, 17, 19));
    }

    Mat dst;
    u.copyTo(dst);

    Mat expected(dst.size(), dst.type(), Scalar(11, 13, 17, 19));
    EXPECT_LE(cvtest::norm(dst, expected, NORM_INF), 0);
}
