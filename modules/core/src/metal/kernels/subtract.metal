...
kernel void subtractKernel(device const uchar* src1 [[buffer(0)]],
                           device const uchar* src2 [[buffer(1)]],
                           device uchar* dst [[buffer(2)]],
                           constant SubtractParams& p [[buffer(3)]],
                           uint2 gid [[thread_position_in_grid]])
{
    ...
    if (p.depth == 0)
    {
        ...
        {
            int diff = (int)src1[src1Base + c] - (int)src2[src2Base + c];
            dst[dstBase + c] = (uchar)max(diff, 0);
        }
    }
    else
    {
