...
DEFINE_SUM_SIMD_16(fp16_t, float, int, v_float32, vx_load_expand)
...
static int sum16f( const fp16_t* src, const uchar* mask, float* dst, int len, int cn )
{ CV_INSTRUMENT_REGION(); return sum_<fp16_t, float, float>(src, mask, dst, len, cn); }
