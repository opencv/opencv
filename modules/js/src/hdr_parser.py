...
class CppHeaderParser(object):
    ...
    def parse_func_decl(self, decl_str):
        ...
        if '&' in  decl_start:
            func_modlist.append("/Ref")
        if 'const' in  decl_start:
            func_modlist.append("/CRet")
        rettype, funcname, modlist, argno = self.parse_arg(decl_start, -1)

        #saji rettype = decl_start.replace( decl_start.split()[-1] , '' ).strip()
        ...
        balance = 1
        angle_balance = 0
        # scan the argument list; handle nestedf parentheses
        args_decls = []
        args = []
        argno = 1
        ...
        while balance > 0:
            ...
            if (t == "," and balance == 1 and angle_balance == 0) or balance == 0:
                ...
                if a:
                    ...
                    #saji
                    #arg_type = a[:a.rfind(" ")]
                    arg_type =arg_type.replace("CV_OUT","")
                    arg_type =arg_type.replace("CV_IN_OUT","")

                    if self.wrap_mode:
                        #if arg_type == "InputArray":
                        #    arg_type = "Mat"
                        if arg_type == "InputOutputArray":
                            #arg_type = "Mat"
                            modlist.append("/IO")
