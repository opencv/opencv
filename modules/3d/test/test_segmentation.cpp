...
class PlaneSegmentationTest : public ::testing::Test
{
public:
    ...
    Mat pt_cloud, generated_pts, detected_plane_models;
    vector<int> label;
...
TEST_F(PlaneSegmentationTest, SinglePlaneDetection)
{
    ...
    for (int i = 0; i < models_num; i++)
    {
        ...
        int num = sacSegmentation.segment(pt_cloud, label, detected_plane_models);
        ...
        Mat ans_model(1, 4, CV_32F, models[i].data()), detected_model(detected_plane_models);
        normalizeMat(ans_model);
        normalizeMat(detected_model);

        EXPECT_LE(getDiff(ans_model, detected_model), 0.0001)
        << "Initial model is " << ans_model << ". Detected model is " << detected_model
...
TEST_F(PlaneSegmentationTest, MultiplePlaneDetection)
{
    ...
    for (int i = 0; i < models_num; i++)
    {
        Mat ans_model(1, 4, CV_32F, models[models_num - 1 - i].data()), detected_model(
                detected_plane_models.row(i));
        normalizeMat(ans_model);
        normalizeMat(detected_model);
...
TEST_F(PlaneSegmentationTest, PlaneDetectionWithConstraints)
{
    sacSegmentation.setMaxIterations(5000);
    sacSegmentation.setDistanceThreshold(0.51);
    sacSegmentation.setNumberOfModelsExpected(1);

    for (int i = 0; i < models_num; i++)
    {
        generatePlane(generated_pts, models[i], 0.5f, pt_nums[i], limits[i]);
        pt_cloud.push_back(generated_pts);
    }
    ...
    for (int i = 0; i < models_num; i++)
    {
        ...
        vector<float> normal_constraint = {models[i][0], models[i][1], models[i][2]};

        // Normal vector constraint function
        ModelConstraintFunction constraint = [normal_constraint, radian_thr](
                const vector<double> &model) -> bool {
            vector<float> model_normal = {(float) model[0], (float) model[1], (float) model[2]};
            float dot12 = normal_constraint[0] * model_normal[0] +
                          normal_constraint[1] * model_normal[1] +
                          normal_constraint[2] * model_normal[2];
            float m1m1 = normal_constraint[0] * normal_constraint[0] +
                         normal_constraint[1] * normal_constraint[1] +
                         normal_constraint[2] * normal_constraint[2];
        ...
        // Single plane detection with constraint
        int num = sacSegmentation.segment(pt_cloud, label, detected_plane_models);
