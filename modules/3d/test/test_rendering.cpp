...
TEST_P(RenderingTest, modelRenderingTest)
{
    Mat depth_buf(height, width, CV_32F, zFar);
    Mat color_buf(height, width, CV_32FC3, Scalar(0.0, 0.0, 0.0));
    string object_path = "../../../opencv_extra/opencv_extra/testdata/rendering/model/spot.obj";
    //position = Vec3f(0.0, 0.0, 10.0);
    position = Vec3f(20, 60, -40.0);
    ...
    std::vector<Vec3f> colors;
    loadMesh(object_path, vertices, colors, indices);

    Mat cameraMatrix(4, 3, CV_32F);
    for (auto& color : colors)
        //color = Vec3f(150.0, 10.0, 238.0);
    ...
    imwrite("model_image.png", color_buf);
    imwrite("model_image_depth.png", depth_buf);
