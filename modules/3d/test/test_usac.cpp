...
TEST(usac_Homography, accuracy) {
    ...
    const std::vector<int> flags = {cv3d::USAC_DEFAULT, cv3d::USAC_ACCURATE, cv3d::USAC_PROSAC, cv3d::USAC_FAST, cv3d::USAC_MAGSAC};
    for (double inl_ratio = 0.1; inl_ratio < 0.91; inl_ratio += 0.1) {
        ...
        for (auto flag : flags) {
            cv::Mat mask, H = cv3d::findHomography(pts1, pts2,flag, thr, mask,
                                                       int(max_iters), conf);
...
TEST(usac_Fundamental, accuracy) {
    ...
    const std::vector<int> flags = {cv3d::USAC_DEFAULT, cv3d::USAC_FM_8PTS, cv3d::USAC_ACCURATE, cv3d::USAC_PROSAC, cv3d::USAC_FAST, cv3d::USAC_MAGSAC};
    ...
    for (double inl_ratio = 0.25; inl_ratio < 0.91; inl_ratio += 0.1) {
        ...
        for (auto flag : flags) {
            const int sample_size = flag == cv3d::USAC_FM_8PTS ? 8 : 7;
            const double max_iters = 1.25 * log(1 - conf) /
                    log(1 - pow(inl_ratio, sample_size));
            cv::Mat mask, F = cv3d::findFundamentalMat(pts1, pts2,flag, thr, conf,
                                                           int(max_iters), mask);
...
TEST(usac_Essential, accuracy) {
    ...
    for (double inl_ratio = 0.35; inl_ratio < 0.91; inl_ratio += 0.1) {
        ...
        for (auto flag : flags) {
            cv::Mat mask, E;
            try {
                E = cv3d::findEssentialMat(pts1, pts2, K1, flag, conf, thr, mask);
...
TEST(usac_P3P, accuracy) {
    ...
    for (double inl_ratio = 0.1; inl_ratio < 0.91; inl_ratio += 0.1) {
        ...
        for (auto flag : flags) {
            ...
            CV_Assert(cv3d::solvePnPRansac(obj_pts, img_pts, K1, cv::noArray(), rvec, tvec,
...
TEST (usac_Affine2D, accuracy) {
    ...
    for (double inl_ratio = 0.1; inl_ratio < 0.91; inl_ratio += 0.1) {
        ...
        const double conf = 0.99, thr = 2., max_iters = 1.3 * log(1 - conf) /
                log(1 - pow(inl_ratio, 3 /* sample size */));
        for (auto flag : flags) {
            cv::Mat mask, A = cv3d::estimateAffine2D(pts1, pts2, mask, flag, thr, (size_t)max_iters, conf, 0);
...
TEST(usac_testUsacParams, accuracy) {
    ...
    const int pts_size = 1500;
    cv::RNG &rng = cv::theRNG();
    const cv3d::UsacParams usac_params = cv3d::UsacParams();
    ...
    model = cv3d::findHomography(pts1, pts2, mask, usac_params);
    ...
    model = cv3d::findFundamentalMat(pts1, pts2, mask, usac_params);
    ...
    try {
        model = cv3d::findEssentialMat(pts1, pts2, K1, K2, dist_coeff, dist_coeff, mask, usac_params);
        cv::Mat cpts1_3d, cpts2_3d;
        cv::vconcat(pts1, cv::Mat::ones(1, pts1.cols, pts1.type()), cpts1_3d);
        cv::vconcat(pts2, cv::Mat::ones(1, pts2.cols, pts2.type()), cpts2_3d);
    ...
    CV_Assert(cv3d::solvePnPRansac(pts2, pts1, K1, dist_coeff, rvec, tvec, mask, usac_params));
    cv3d::Rodrigues(rvec, R); cv::hconcat(K1 * R, K1 * tvec, model);
    ...
    cv::Mat K_est;
    CV_Assert(cv3d::solvePnPRansac(pts2, pts1, K_est, dist_coeff, rvec, tvec, mask, usac_params));
    cv3d::Rodrigues(rvec, R); cv::hconcat(K_est * R, K_est * tvec, model);
    checkInliersMask(TestSolver::PnP, inl_size, usac_params.threshold, pts1, pts2, model, mask);
    ...
    model = cv3d::estimateAffine2D(pts1, pts2, mask, usac_params);
