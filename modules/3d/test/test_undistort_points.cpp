...
void CV_UndistortTest::run(int /* start_from */)
{
    ...
    for (int idx = 0; idx < 3; idx++)
    {
        ...
        cv3d::projectPoints(Mat(points), Mat::zeros(3,1,CV_64FC1), Mat::zeros(3,1,CV_64FC1), intrinsics, distCoeffs, projectedPoints);

        vector<Point2f> realUndistortedPoints;
        cv3d::projectPoints(Mat(points), Mat::zeros(3,1,CV_64FC1), Mat::zeros(3,1,CV_64FC1), intrinsics,  Mat::zeros(4,1,CV_64FC1), realUndistortedPoints);

        Mat undistortedPoints;
        cv3d::undistortPoints(Mat(projectedPoints), undistortedPoints, intrinsics, distCoeffs);
...
TEST(Calib3d_Undistort, stop_criteria)
{
    ...
    cv3d::undistortPoints(pt_distorted_vec, pt_undist_vec, cameraMatrix, distCoeffs, noArray(), noArray(), criteria);
    ...
    cv3d::projectPoints(pt_undist_vec_homogeneous, Mat::zeros(3,1,CV_64F), Mat::zeros(3,1,CV_64F), cameraMatrix, distCoeffs, pt_redistorted_vec);
...
TEST(undistortPoints, regression_14583)
{
    ...
    cv::Mat undistort_pt;
    cv3d::undistortPoints(distort_pt, undistort_pt, camera_interior,
