...
void usacConfigToModelParams (const Ptr<const UsacConfig> &config, Ptr<Model> &params);
...
public:
    SimpleUsacConfigImpl(EstimationMethod est_method_) :
                             est_method(est_method_),
                             max_iterations(2500),
    ...
    EstimationMethod getEstimationMethod () const override { return est_method; }
...
Ptr<SimpleUsacConfig> SimpleUsacConfig::create(EstimationMethod est_method) {
    return makePtr<SimpleUsacConfigImpl>(est_method);
}

UniversalRANSAC::UniversalRANSAC (Ptr<Model> &params_, cv::InputArray points1, cv::InputArray points2,
    ...
    if (params->isPnP()) {
        ...
        points = points1.getMat();
        points_size = points.rows * points.cols / 3;
    } else {
...
void UniversalRANSAC::initialize (int state, Ptr<MinimalSolver> &min_solver, Ptr<NonMinimalSolver> &non_min_solver,
    ...
    switch (params->getError()) {
        ...
        case ErrorMetric::POINT_TO_PLANE:
            error = PlaneModelError::create(points); break;
        case ErrorMetric::POINT_TO_SPHERE:
            error = SphereModelError::create(points); break;
    ...
    if (params->isHomography()) {
        ...
        if (params->getEstimator() == EstimationMethod::PLANE) {
            ...
            _fo_solver = SphereModelNonMinimalSolver::create(points);
            const double _radius_min = DBL_MIN, _radius_max = DBL_MAX;
...
UniversalRANSAC::UniversalRANSAC (const Ptr<const UsacConfig> &config_, int points_size_,
        Ptr<Estimator> &estimator_, const Ptr<Quality> &quality_,
        ...
        Ptr<FinalModelPolisher> &model_polisher_) :

        config (config_), _estimator (estimator_), _error(quality_->getErrorFnc()),
        _quality (quality_), _sampler (sampler_), _termination (termination_criteria_), _model_verifier (model_verifier_),
        _degeneracy (degeneracy_), _local_optimization (local_optimization_),
        polisher (model_polisher_), points_size (points_size_) {
        usacConfigToModelParams(config_, params);
        if (model_polisher_ == nullptr)
            params->setPolisher(PolishingMethod::NONE_POLISHER);
        if (local_optimization_ == nullptr)
            params->setLocalOptimization(LocalOptimMethod::LOCAL_OPTIM_NULL);
        threshold = quality_->getThreshold();
        parallel = params->isParallel();
        _state = params->getRandomGeneratorState();
        if (parallel)
            CV_Error(cv::Error::StsNotImplemented, "Parallel version is available only if initialized via different constructor!");
}

int UniversalRANSAC::getIndependentInliers (const Mat &model_, const std::vector<int> &sample,
...
Ptr<Quality> UniversalRANSAC::getQuality() const {
    return _quality;
}
int UniversalRANSAC::getPointsSize() const {
    return points_size;
}
const Mat &UniversalRANSAC::getK1() const {
    return K1;
}
...
public:
    ModelImpl (double threshold_, EstimationMethod estimator_, SamplingMethod sampler_, double confidence_,
        ...
        switch (estimator_) {
            ...
            case (EstimationMethod::PLANE):
                avg_num_models = 1; model_est_to_ver_time = 100;
                sample_size = 3; est_error = ErrorMetric ::POINT_TO_PLANE; break;
            case (EstimationMethod::SPHERE):
                avg_num_models = 1; model_est_to_ver_time = 100;
                sample_size = 4; est_error = ErrorMetric ::POINT_TO_SPHERE; break;
...
void usacConfigToModelParams (const Ptr<const UsacConfig> &config, Ptr<Model> &params) {
    params = Model::create(1.0, config->getEstimationMethod(), config->getSamplingMethod(), 0.99, config->getMaxIterations(), config->getScoreMethod());
    params->setRandomGeneratorState(config->getRandomGeneratorState());
    params->setParallel(config->isParallel());
    params->setNeighborsType(config->getNeighborsSearchMethod());
    params->setLocalOptimization(config->getLOMethod());
    params->maskRequired(config->isMaskRequired());
}

void modelParamsToUsacConfig (Ptr<SimpleUsacConfig> &config, const Ptr<const Model> &params) {
    config = SimpleUsacConfig::create(params->getEstimator());
...
bool run (Ptr<Model> &params, InputArray points1, InputArray points2,
    ...
    UniversalRANSAC ransac (params, points1, points2, K1_, K2_, dist_coeff1, dist_coeff2);
    if (ransac.run(ransac_output)) {
        if (params->isPnP()) {
            ...
            std::vector<bool> inliers_mask(ransac.getPointsSize());
            ransac.getQuality()->getInliers(newP, inliers_mask);
