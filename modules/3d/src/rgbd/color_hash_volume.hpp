...
void fetchPointsNormalsColorsFromHashTsdfVolumeUnit(
    const VolumeSettings& settings, InputArray _volUnitsData, const VolumeUnitIndexes& volumeUnits,
...
void ocl_fetchPointsNormalsColorsFromHashTsdfVolumeUnit(
    const VolumeSettings& settings, const int volumeUnitDegree, InputArray _volUnitsData, InputArray _volUnitsDataCopy,
...
#endif // OPENCV_3D_HASH_TSDF_FUNCTIONS_HPP


#ifndef __OPENCV_COLOR_HASH_VOLUME_HPP__
#define __OPENCV_COLOR_HASH_VOLUME_HPP__

#include <opencv2/3d/volume.hpp>

namespace cv {

class CV_EXPORTS_W ColorHashTSDFVolume
{
public:
    virtual ~ColorHashTSDFVolume() {}

    virtual void integrate(InputArray depth, InputArray color, 
                         InputArray cameraPose, InputArray cameraMat) = 0;
    
    virtual void raycast(InputArray cameraPose, InputArray cameraMat,
                        OutputArray points, OutputArray normals, OutputArray colors,
                        Size frameSize) = 0;

    static Ptr<ColorHashTSDFVolume> create(const VolumeSettings& settings);
};
