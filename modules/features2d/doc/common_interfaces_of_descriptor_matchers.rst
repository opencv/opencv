...
.. ocv:function:: void DescriptorMatcher::radiusMatch( const Mat& queryDescriptors,           vector<vector<DMatch> >& matches,           float maxDistance,      const vector<Mat>& masks=vector<Mat>(),       bool compactResult=false )
    ...
    :param maxDistance: Threshold for the distance between matched descriptors. Distance means metric distance (e.g. Hamming distance), not the distance between coordinates which is measured in Pixels!
