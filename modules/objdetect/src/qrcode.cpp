...
//  a/    | c
    ...
    decode_error QRDecode::read_format(uint16_t& format , int which){
        ...
        std::cout <<"adj:"<<D2B(my_format)<<std::endl;
        std::cout<<"FINAL : \n"<<mat_format<<std::endl;
        format=my_format;
 ...
 * 格式码纠错*/
    void QRDecode::read_bit(int x, int y, int& count){
        /*judge the reserved area 判断是不是保留字*/
        if (unmasked_data.ptr(y)[x]==INVALID_REGION)
            return ;
        /*the bitpos^th bit of the  bytepos^th codeword 当前第（bytepos）个数据块的第（bitpos）个bit*/
    ...
    void QRDecode:: unmask_data(){
        ...
        for(int i= 0;i<size;i++){
            for(int j= 0;j<size;j++){
                ...
                /*version information 尚未读出version信息*/
                else if (version >= 7) {
