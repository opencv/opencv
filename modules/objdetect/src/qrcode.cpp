...
{
    QRDecode qrdec;
    qrdec.init(in.getMat());
    qrdec.binarization();
    if (!qrdec.localization()) { cout << "Localization failed" << '\n'; }
    if (!qrdec.transformation()) { cout << "Transformation failed" << '\n'; };
    points.setTo(qrdec.getTransformationPoints());
    out.setTo(qrdec.getStraightBarcode());
    decode = String();
}

void QRDecode::init(Mat src,
                    double eps_vertical_,
                    double eps_horizontal_)
{
    barcode = src;
    eps_vertical   = eps_vertical_;
    eps_horizontal = eps_horizontal_;
    show_radius = (barcode.rows > barcode.cols)
         ? 3.375 * barcode.rows / barcode.cols
         : 3.375 * barcode.cols / barcode.rows;
}
...
bool QRDecode::localization()
{
    ...
    vector<Vec3d> list_lines_y = separateHorizontalLines(list_lines_x);
    vector<Vec3d> result_point = pointClustering(list_lines_y);
    for (int i = 0; i < 3; i++)
    {
        localization_points.push_back(
            Point(result_point[i][0], result_point[i][1] + result_point[i][2]));
    ...
    // circle(local_barcode, localization_points[0], show_radius, Scalar(0, 0, 255), -1);
    // circle(local_barcode, localization_points[1], show_radius, Scalar(0, 255, 0), -1);
    // circle(local_barcode, localization_points[2], show_radius, Scalar(255, 0, 0), -1);
...
vector<Vec3d> QRDecode::separateHorizontalLines(vector<Vec3d> list_lines)
{
    result.clear();
    int temp_length = 0;
    int x, y;
    ...
    {
        x = list_lines[pnt][0] + list_lines[pnt][2] / 2;
        y = list_lines[pnt][1];
...
vector<Vec3d> QRDecode::pointClustering(vector<Vec3d> list_lines)
{
    vector<Vec3d> centers;
    vector<Point> clusters[3];
    double weight_clusters[3] = {0.0, 0.0, 0.0};
    Point basis[3], temp_pnt;
    double temp_norm = 0.0, temp_compute_norm, distance[3];

    basis[0] = Point(list_lines[0][1], list_lines[0][0]);
    for (size_t i = 1; i < list_lines.size(); i++)
    {
        temp_pnt = Point(list_lines[i][1], list_lines[i][0]);
...
void QRDecode::fixationPoints(vector<Point> &local_point, vector<double> &local_len)
{
    ...
    vector_mult.at<float>(0, 1) = (local_point[1] - local_point[0]).x;
    vector_mult.at<float>(1, 1) = (local_point[1] - local_point[0]).y;
    vector_mult.at<float>(0, 2) = (local_point[2] - local_point[0]).x;
    vector_mult.at<float>(1, 2) = (local_point[2] - local_point[0]).y;
...
bool QRDecode::transformation()
{
    ...
    circle(transform_barcode, localization_points[0], show_radius, Scalar(0, 0, 255), -1);
    circle(transform_barcode, localization_points[1], show_radius, Scalar(0, 255, 0), -1);
    circle(transform_barcode, localization_points[2], show_radius, Scalar(255, 0, 0), -1);
    ...
    vector<Point> quadrilateral = getQuadrilateral(transformation_points);
    transformation_points = quadrilateral;

    vector<vector<Point>> full_quadrilateral;
    full_quadrilateral.push_back(quadrilateral);
    drawContours(transform_barcode, full_quadrilateral, 0, Scalar(35, 142, 107),
    show_radius, 8, vector<Vec4i>(), 0, Point());

    circle(transform_barcode, transformation_points[0], show_radius, Scalar(0, 0, 255), -1);
    circle(transform_barcode, transformation_points[1], show_radius, Scalar(0, 255, 0), -1);
    circle(transform_barcode, transformation_points[2], show_radius, Scalar(255, 0, 255), -1);
    circle(transform_barcode, transformation_points[3], show_radius, Scalar(255, 0, 0), -1);

    int max_length_norm = -1;
    int transform_size = transformation_points.size();
    for (int i = 0; i < transform_size; i++)
    {
        int len_norm = norm(transformation_points[i % transform_size] -
                            transformation_points[(i + 1) % transform_size]);
        if (max_length_norm < len_norm) { max_length_norm = len_norm; }
    }

    vector<Point> perspective_points = {
        Point(0, 0),
        Point(0, max_length_norm),
        Point(max_length_norm, max_length_norm),
        Point(max_length_norm, 0),
    };
                                       ...
                                       bool right_rotate)
{
    ...
    if (cos_angle_rotation == 0.0)
    {
        ...
        vector_mult.at<float>(1, 0) = 1;
        vector_mult.at<float>(2, 0) = 1;
        vector_mult.at<float>(0, 1) = (left - center).x;
        vector_mult.at<float>(1, 1) = (left - center).y;
        vector_mult.at<float>(0, 2) = (left - start_pnt).x;
        vector_mult.at<float>(1, 2) = (left - start_pnt).y;
        double res_vect_mult = determinant(vector_mult);
...
vector<Point> QRDecode::getQuadrilateral(vector<Point> angle_list)
{
    int angle_size = angle_list.size();
    ...
    vector<vector<Point>> hull(1), approx_hull(1);
    convexHull(Mat(locations), hull[0]);
    int hull_size = hull[0].size();

    Point min_pnt;

    vector<Point> min_abc;
    ...
    int min_abc_size = min_abc.size();
    vector<int> index_min_abc(min_abc_size);
    ...
    vector<Point> result_hull_point(angle_size);
    double min_norm, temp_norm;
    for (int i = 0; i < angle_size; i++)
    {
        min_norm = numeric_limits<double>::max();
        Point closest_pnt;
    ...
    if (min_norm == numeric_limits<double>::max())
    {
    ...
    vector<Point> result_angle_list(4), test_result_angle_list(4);
