                    ...
                    Rect* roi1, Rect* roi2 )
{
    ...
    if( matR.rows == 3 && matR.cols == 3 )
        cv3d::Rodrigues(matR, om);          // get vector rotation
    else
        matR.copyTo(om);
    om *= -0.5; // get average rotation
    cv3d::Rodrigues(om, r_r);
    ...
    for( int k = 0; k < 2; k++ )
    {
        ...
        cv3d::undistortPoints(pts, pts, A, Dk, Mat(), Mat());
        cv3d::convertPointsToHomogeneous(pts, pts_3);
        ...
        cv3d::projectPoints(pts_3, (k == 0 ? _R1 : _R2), Z, A_tmp, Mat(), pts);
        Scalar avg = mean(pts);
    ...
    cv3d::getUndistortRectangles(cameraMatrix1, distCoeffs1, _R1, _P1, imageSize, inner1, outer1);
    cv3d::getUndistortRectangles(cameraMatrix2, distCoeffs2, _R2, _P2, imageSize, inner2, outer2);
                                ...
                                OutputArray _Hmat1, OutputArray _Hmat2, double threshold )
{
    ...
    {
        Mat _lines1, _lines2;
        cv3d::computeCorrespondEpilines(_m1, 1, F, _lines1);
        cv3d::computeCorrespondEpilines(_m2, 2, F, _lines2);
    ...
    Mat A, X;
    cv3d::convertPointsToHomogeneous(_m1, A, CV_64F);
                            ...
                            const Mat& R1, const Mat& R3, const Mat& P1, Mat& P3 )
{
    ...
    cv3d::undistortPoints(imgpt1, imgpt1, cameraMatrix1, distCoeffs1, R1, P1);
    cv3d::undistortPoints(imgpt3, imgpt3, cameraMatrix3, distCoeffs3, R3, P3);
                   ...
                   Rect* roi1, Rect* roi2, int flags )
{
    ...
    if( R13.size() != Size(3,3) )
        cv3d::Rodrigues(R13, r_r13);
    else
        R13.copyTo(r_r13);

    if( R12.size() == Size(3,3) )
        cv3d::Rodrigues(R12, om);
    else
        R12.copyTo(om);

    om *= -0.5;
    cv3d::Rodrigues(om, r_r); // rotate cameras to same orientation by averaging
