...
def intersectionRate(s1, s2):
    ...
    return old_div(2 * area, (cv.contourArea(np.array(s1)) + cv.contourArea(np.array(s2))))
...
class squares_test(NewOpenCVTests):
    ...
    def test_squares(self):
        ...
        self.assertGreater(old_div(matches_counter, len(testSquares)), 0.9)
        self.assertLess( old_div((len(squares) - matches_counter), len(squares)), 0.2)
