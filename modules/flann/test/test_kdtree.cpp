 ...
 * ------------------------
 * Exact KNN (FLANN_CHECKS_UNLIMITED) and radius search (FLANN_CHECKS_UNLIMITED)
 * must return perfect results — max_exact_wrong and max_radius_wrong are 0.
 * The correctness fix in searchLevelExact (replace-not-accumulate lower-bound
 * per Arya & Mount 1993) is required for these tests to pass.
 * Approximate KNN (checks=32) is inherently lossy; those budgets remain non-zero.
 */
...
struct KDTreeTestCase {
    ...
    unsigned seed;
    float dist_range;
    // Maximum acceptable failure counts for each search mode.
    // Values are chosen based on the expected accuracy of the patched
    // implementation for each dataset configuration.
    int max_exact_wrong;
    int max_approx_wrong;
    int max_radius_wrong;
};
...
TEST_P(Flann_KDTree_Accuracy, regression)
{
    ...
    EXPECT_LE(exact_wrong,  tc.max_exact_wrong)
        << tc.name << ": exact KNN exceeded acceptable error budget";
    EXPECT_LE(approx_wrong, tc.max_approx_wrong)
        << tc.name << ": approx KNN exceeded acceptable error budget";
    EXPECT_LE(radius_wrong, tc.max_radius_wrong)
        << tc.name << ": radius search exceeded acceptable error budget";
}

// exact and radius budgets are 0: FLANN_CHECKS_UNLIMITED must be perfect.
// approx budgets tolerate the inherent miss rate of checks=32.
static const KDTreeTestCase kAccuracyCases[] = {
    // name,               N,      dim, k,  n_q,  radius,  seed, range, exact, approx, radius_w
    { "standard_3D",       10000,  3,   10, 50,   200,     42,   1000,  0,     107,    0   },
    { "small_dataset",     15,     3,   5,  200,  50,      7,    500,   0,     20,     0   },
    { "k_1",               5000,   3,   1,  30,   200,     11,   1000,  0,     3,      0   },
    { "large_k_50",        1000,   3,   50, 100,  50,      17,   500,   0,     110,    0   },
    { "k_eq_N",            20,     3,   20, 9999, 30,      23,   100,   0,     0,      0   },
    { "high_dim_64",       2000,   64,  10, 50,   50,      31,   10,    0,     50,     0   },
    { "large_radius",      5000,   3,   10, 2000, 50,      37,   1000,  0,     2000,   0   },
    { "tiny_radius",       5000,   3,   10, 1,    100,     41,   1000,  0,     60,     0   },
    { "dim_2D",            8000,   2,   10, 30,   200,     53,   500,   0,     74,     0   },
    { "many_trees_4",      5000,   3,   10, 50,   100,     59,   1000,  0,     58,     0   },
};
