...
struct Trait<schar>
{
    static inline bool checkValue(const schar* elem, const schar*) { return *elem != 0; }
    static inline bool isVal(const schar* elem, const schar*) { return *elem == MASK8_BLACK; }
    ...
    static inline void setNewFlag(schar* elem, const schar*, schar nbd) { *elem = nbd; }
};
...
template <typename T>
static void icvFetchContourEx(Mat& image, const Point& start, T nbd, Contour& res_contour,
                              const bool isDirect)
{
    const size_t step = image.step1();
...
shared_ptr<ContourScanner_> ContourScanner_::create(Mat img, int mode, int method, Point offset)
{
    ...
    if (mode == RETR_FLOODFILL)
        ...
        CV_CheckTypeEQ(
            img.type(), CV_8UC1,
            "Modes other than RETR_FLOODFILL and RETR_CCOMP support only CV_8UC1 images");
...
int ContourScanner_::findFirstBoundingContour(const Point& last_pos, const int y, const int lval,
                                              int par)
{
...
void cv::findContours(InputArray _image, OutputArrayOfArrays _contours, OutputArray _hierarchy,
                      int mode, int method, Point offset)
{
    ...
    if (method == 5 /*CV_LINK_RUNS*/)
    {
        CV_LOG_ONCE_WARNING(
            NULL,
            "LINK_RUNS mode has been extracted to separate function: cv::findContoursLinkRuns. "
    ...
    {
        CV_LOG_ONCE_WARNING(
            NULL, "Chain code output is an experimental feature and might change in future!");
    }
...
void cv::findContours(InputArray _image, OutputArrayOfArrays _contours, int mode, int method,
                      Point offset)
{
    CV_INSTRUMENT_REGION();
