...
void cv::contourTreeToResults(CTree& tree, int res_type, OutputArrayOfArrays& _contours,
                              OutputArray& _hierarchy)
{
    ...
    if (_hierarchy.needed())
    {
        ...
        while (!it.isDone())
        {
            ...
            h_vec = Vec4i(index_mapping.at(elem.next), index_mapping.at(elem.prev),
                          index_mapping.at(elem.first_child), index_mapping.at(elem.parent));
            ++i;
        }
    }
}
