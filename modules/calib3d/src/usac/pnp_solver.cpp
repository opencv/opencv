...
namespace cv
{
    namespace usac
    {
        ...
        public:
            ...
            explicit PnPMinimalSolver6PtsImpl(const Mat &points_) : points_mat(&points_), points((float *)points_.data) {}
            /*
        DLT:
    ...
    */
        ...
        public:
            explicit PnPNonMinimalSolverImpl(const Mat &points_) : points_mat(&points_), points((float *)points_.data) {}
                         ...
                         std::vector<Mat> &models, const std::vector<double> &weights) const override
            {
                ...
                Vec<double, 12> D;
                if (!eigen(Matx<double, 12, 12>(AtA), D, Vt))
                    return 0;
     ...
     */
            P3PSolverImpl(const Mat &points_, const Mat &calib_norm_points_, const Mat &K_) : points_mat(&points_), calib_norm_points_mat(&calib_norm_points_), K_mat(&K_),
         ...
         */
                ...
                for (double root : roots)
                {
                    ...
                    const double n13 = (d12_p2 * (d23_p2 - d31_p2 * n12_p2) + (d23_p2 - d31_p2) * (d23_p2 * (1 + n12_p2 - 2 * n12 * c12) - d12_p2 * n12_p2)) / (2 * d12_p2 * (d23_p2 * c31 - d31_p2 * c23 * n12) + 2 * (d31_p2 - d23_p2) * d12_p2 * c23 * n12);
