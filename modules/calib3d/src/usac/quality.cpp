...
namespace cv
{
    namespace usac
    {
        ...
        public:
                  ...
                  stored_lower_incomplete_gamma_values(gamma_generator.getIncompleteGammaValues())
            {
                ...
                {
                    int x = (int)round(scale_of_stored_incomplete_gammas * sqr_res / maximum_sigma_2_times_2);
                    ...
                    const double loss = two_ad_dof_plus_one_per_maximum_sigma * (maximum_sigma_2_per_2 *
                                                                                     stored_lower_incomplete_gamma_values[x] +
                                                                                 sqr_res * 0.25 *
            ...
            Score getScore(const Mat &model) const override
            {
                ...
                for (int point_idx = 0; point_idx < points_size; point_idx++)
                {
                    ...
                    { // consider point as inlier
                        // Get the position of the gamma value in the lookup table
                        int x = (int)round(scale_of_stored_incomplete_gammas * squared_residual / maximum_sigma_2_times_2);
                        ...
                        total_loss -= (1 - two_ad_dof_plus_one_per_maximum_sigma * (maximum_sigma_2_per_2 * stored_lower_incomplete_gamma_values[x] + squared_residual * 0.25 * (stored_complete_gamma_values[x] - gamma_value_of_k)) / max_loss);
                    }
        ...
        public:
            LMedsQualityImpl(int points_size_, double threshold_, const Ptr<Error> &error_) : error(error_), points_size(points_size_), threshold(threshold_) {}
        ...
        public:
                                                                                           ...
                                                                                           m_S(avg_num_models), score_type(score_type_)
            {
                ...
                can_compute_score = score_type_ == ScoreMethod::SCORE_METHOD_MSAC || score_type_ == ScoreMethod::SCORE_METHOD_RANSAC || score_type_ == ScoreMethod::SCORE_METHOD_LMEDS;
     ...
     */
            inline bool isModelGood(const Mat &model) override
            {
                ...
                for (tested_point = 0; tested_point < points_size; tested_point++)
                {
                    ...
                    else if (score_type == ScoreMethod::SCORE_METHOD_RANSAC)
                    {
                        ...
                        errors[points_random_pool[random_pool_idx - 1]] = (float)error;
                }
             ...
             */
                    ...
                    if (delta_estimated > 0 && fabs(current_delta - delta_estimated) / current_delta > 0.05)
                        /*
