...
*/
...
#endif
        public:
            explicit EssentialMinimalSolverStewenius5ptsImpl(const Mat &points_) : points_mat(&points_)
#if defined(HAVE_EIGEN) || defined(HAVE_LAPACK)
                                                                                   ,
            ...
            int estimate(const std::vector<int> &sample, std::vector<Mat> &models) const override
            {
                ...
                Mat(multPolysDegOneAndTwo(
                        (multPolysDegOne(null_space_mat[0][1].val, null_space_mat[1][2].val) -
                         multPolysDegOne(null_space_mat[0][2].val, null_space_mat[1][1].val))
                            .val,
                        null_space_mat[2][0].val) +
                    multPolysDegOneAndTwo(
                        (multPolysDegOne(null_space_mat[0][2].val, null_space_mat[1][0].val) -
                         multPolysDegOne(null_space_mat[0][0].val, null_space_mat[1][2].val))
                            .val,
                        null_space_mat[2][1].val) +
                    multPolysDegOneAndTwo(
                        (multPolysDegOne(null_space_mat[0][0].val, null_space_mat[1][1].val) -
                         multPolysDegOne(null_space_mat[0][1].val, null_space_mat[1][0].val))
                            .val,
                        null_space_mat[2][2].val))
                    .copyTo(constraint_mat.row(9));
                ...
                const Eigen::Matrix<double, 10, 10> eliminated_mat_eig = constraint_mat_eig.block<10, 10>(0, 0)
                                                                             .fullPivLu()
                                                                             .solve(constraint_mat_eig.block<10, 10>(0, 10));
                ...
                if (!solve(A, B, eliminated_mat, DECOMP_LU))
                    return 0;
     ...
     */
            ...
            {
                return {p[0] * q[0], p[0] * q[1] + p[1] * q[0], p[1] * q[1], p[0] * q[2] + p[2] * q[0], p[1] * q[2] + p[2] * q[1],
     ...
     */
            static inline Matx<double, 1, 20> multPolysDegOneAndTwo(const double *const p,
                                                                    const double *const q)
            {
                return Matx<double, 1, 20>({p[0] * q[0], p[0] * q[1] + p[1] * q[0], p[1] * q[1] + p[2] * q[0], p[2] * q[1], p[0] * q[2] + p[3] * q[0],
        ...
        Ptr<EssentialMinimalSolverStewenius5pts> EssentialMinimalSolverStewenius5pts::create(const Mat &points_)
        {
     ...
     */
            explicit EssentialNonMinimalSolverImpl(const Mat &points_) : points_mat(&points_), points((float *)points_.data) {}
