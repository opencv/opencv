        ...
        public:
                    ...
                    for (; iters < max_iters; iters++)
                    {
                        ...
                        for (int i = 0; i < number_of_models; i++)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    if (!_local_optimization->refineModel(models[i], best_score, models[i], current_score))
                                        continue;
                                }
                            ...
                            if (current_score.isBetter(best_score))
                            {
                                                                     ...
                                                                     non_degenerate_model, non_denegenerate_model_score))
                                {
                                    ...
                                    max_iters = update_best(models[i], current_score);

                                if (LO && iters >= max_iters_before_LO)
                                {
                                    // do magsac if it wasn't already run
                                    if (is_magsac && iters % repeat_magsac == 0 && iters >= max_hyp_test_before_ver)
                                        continue; // magsac has already run
                                    was_LO_run = true;
            ...
            if (!success) { // cover all if not success to avoid thread creating new variables
                ...
                ransac_output = RansacOutput::create(best_model, inliers_mask,
                                                     static_cast<int>(std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - begin_time).count()), best_score.score,
 ...
 */
                           ...
                           int max_iters, double conf, bool mask_needed)
        {
            ...
            case USAC_FM_8PTS:
                ...
                break;
            default:
                CV_Error(cv::Error::StsBadFlag, "Incorrect flag for USAC!");
            }
        ...
        public:
                      ...
                      int max_iterations_ = 5000, ScoreMethod score_ = ScoreMethod::SCORE_METHOD_MSAC)
            {
                ...
                case (EstimationMethod::Affine):
                    avg_num_models = 1;
                    time_for_model_est = 50;
                    sample_size = 3;
                    est_error = ErrorMetric ::FORW_REPR_ERR;
                    break;
                ...
                case (EstimationMethod::Fundamental):
                    avg_num_models = 2.38;
                    time_for_model_est = 180;
                    maximum_thr = 2.5;
                    sample_size = 7;
                    est_error = ErrorMetric ::SAMPSON_ERR;
                    break;
                ...
                case (EstimationMethod::Essential):
                    avg_num_models = 3.93;
                    time_for_model_est = 1000;
                    maximum_thr = 2.5;
                    sample_size = 5;
                    est_error = ErrorMetric ::SGD_ERR;
                    break;
                case (EstimationMethod::P3P):
                    avg_num_models = 1.38;
                    time_for_model_est = 800;
                    sample_size = 3;
                ...
                case (EstimationMethod::P6P):
                    ...
                    break;
                default:
                    CV_Error(cv::Error::StsNotImplemented, "Estimator has not implemented yet!");
                }
                 ...
                 InputArray dist_coeff1, InputArray dist_coeff2)
        {
                ...
                if (!K1_.empty())
                {
                    ...
                    if (!dist_coeff1.empty())
                    {
                        ...
                        if (points1.isContinuous())
                            ...
                            undistortPoints(points1.getMat().clone(), undist_points1, K1_, dist_coeff1);
                ...
                if (params->isEssential())
                {
                    ...
                    if (!dist_coeff1.empty() || !dist_coeff2.empty())
                    {
                        ...
                        if (points2.isContinuous())
                            ...
                            undistortPoints(points2.getMat().clone(), undist_points2, K2_, dist_coeff2);
            ...
            if (params->getSampler() == SamplingMethod::SAMPLING_NAPSAC || params->getLO() == LocalOptimMethod::LOCAL_OPTIM_GC)
            {
                ...
                else if (params->getNeighborsSearch() == NeighborSearchMethod::NEIGH_FLANN_RADIUS)
                {
                    ...
                    CV_Error(cv::Error::StsNotImplemented, "Graph type is not implemented!");
            }
            ...
            if (params->getSampler() == SamplingMethod::SAMPLING_PROGRESSIVE_NAPSAC)
            {
                ...
                for (int i = 0; i < 4 * points_size; i += 4)
                {
                    if (pts[i] > img1_width)
                        img1_width = pts[i];
                    if (pts[i + 1] > img1_height)
                        img1_height = pts[i + 1];
                    if (pts[i + 2] > img2_width)
                        img2_width = pts[i + 2];
                    if (pts[i + 3] > img2_height)
                        img2_height = pts[i + 3];
                }
            ...
            case ErrorMetric::SYMM_REPR_ERR:
                error = ReprojectionErrorSymmetric::create(points);
                break;
            ...
            case ErrorMetric::SAMPSON_ERR:
                error = SampsonError::create(points);
                break;
            case ErrorMetric::SGD_ERR:
                error = SymmetricGeometricDistance::create(points);
                break;
            case ErrorMetric::RERPOJ:
                error = ReprojectionErrorPmatrix::create(points);
                break;
            default:
                CV_Error(cv::Error::StsNotImplemented, "Error metric is not implemented!");
            }
            ...
            case ScoreMethod::SCORE_METHOD_MSAC:
                quality = MsacQuality::create(points_size, threshold, error);
                break;
            case ScoreMethod::SCORE_METHOD_MAGSAC:
                quality = MagsacQuality::create(max_thr, points_size, error,
                                                ...
                                                params->getLowerIncompleteOfSigmaQuantile(), params->getC());
                break;
            case ScoreMethod::SCORE_METHOD_LMEDS:
                quality = LMedsQuality::create(points_size, threshold, error);
                break;
            default:
                CV_Error(cv::Error::StsNotImplemented, "Score is not imeplemeted!");
            }
            ...
            else if (params->isFundamental())
            {
                ...
                if (min_sample_size == 7)
                    min_solver = FundamentalMinimalSolver7pts::create(points);
                else
                    min_solver = FundamentalMinimalSolver8pts::create(points);
            ...
            else if (params->getEstimator() == EstimationMethod::Affine)
            {
                ...
                CV_Error(cv::Error::StsNotImplemented, "Estimator not implemented!");
            ...
            case SamplingMethod::SAMPLING_PROGRESSIVE_NAPSAC:
                sampler = ProgressiveNapsac::create(state++, points_size, min_sample_size, layers, 20);
                break;
            case SamplingMethod::SAMPLING_NAPSAC:
                sampler = NapsacSampler::create(state++, points_size, min_sample_size, graph);
                break;
            default:
                CV_Error(cv::Error::StsNotImplemented, "Sampler is not implemented!");
            }

            switch (params->getVerifier())
            {
            case VerificationMethod::NullVerifier:
            ...
            case VerificationMethod::SprtVerifier:
                verifier = SPRT::create(state++, error, points_size, params->getScore() == ScoreMethod ::SCORE_METHOD_MAGSAC ? max_thr : threshold,
                                        ...
                                        params->getSPRTavgNumModels(), params->getScore());
                break;
            default:
                CV_Error(cv::Error::StsNotImplemented, "Verifier is not imeplemented!");
            }
            ...
            else if (params->getVerifier() == VerificationMethod::SprtVerifier)
            {
                ...
                termination = StandardTerminationCriteria::create(params->getConfidence(), points_size, min_sample_size, params->getMaxIters());
            ...
            if (params->getLO() != LocalOptimMethod::LOCAL_OPTIM_NULL)
            {
                ...
                case LocalOptimMethod::LOCAL_OPTIM_INNER_LO:
                    lo = InnerIterativeLocalOptimization::create(estimator, quality, lo_sampler,
                                                                 ...
                                                                 params->getLOThresholdMultiplier());
                    break;
                ...
                case LocalOptimMethod::LOCAL_OPTIM_GC:
                    lo = GraphCut::create(estimator, error, quality, graph, lo_sampler, threshold,
                                          params->getGraphCutSpatialCoherenceTerm(), params->getLOInnerMaxIters());
                    break;
                case LocalOptimMethod::LOCAL_OPTIM_SIGMA:
                    lo = SigmaConsensus::create(estimator, error, quality, verifier,
                                                ...
                                                params->getUpperIncompleteOfSigmaQuantile(), params->getC(), max_thr);
                    break;
                default:
                    CV_Error(cv::Error::StsNotImplemented, "Local Optimization is not implemented!");
                }
            }
