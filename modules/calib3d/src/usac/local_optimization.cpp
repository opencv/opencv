    ...
    namespace usac
    {
        ...
        public:
            ...
            GraphCutImpl(const Ptr<Estimator> &estimator_, const Ptr<Error> &error_, const Ptr<Quality> &quality_,
                         ...
                         double threshold_, double spatial_coherence_term, int gc_inner_iteration_number_) : neighborhood_graph(neighborhood_graph_), estimator(estimator_), quality(quality_),
                ...
                while (is_best_model_updated)
                {
                    ...
                    for (int iter = 0; iter < lo_inner_iterations; iter++)
                    {
                        ...
                        if (labeling_inliers_size > gc_sample_size)
                        {
                            ...
                            num_of_estimated_models = estimator->estimateModelNonMinimalSample(lo_sampler->generateUniqueRandomSubset(labeling_inliers,
                                                                                                                                      labeling_inliers_size),
                                                                                               gc_sample_size, gc_models, weights);
                        }
                        else
                        {
                            if (iter > 0)
                                break; // break inliers are not updated
                            num_of_estimated_models = estimator->estimateModelNonMinimalSample(labeling_inliers, labeling_inliers_size, gc_models, weights);
                        }
...
*/
        ...
        public:
            InnerIterativeLocalOptimizationImpl(const Ptr<Estimator> &estimator_, const Ptr<Quality> &quality_,
                ...
                : estimator(estimator_), quality(quality_), lo_sampler(lo_sampler_), lo_iter_sample_size(0), new_threshold(0), threshold_step(0)
            {
     ...
     */
                             ...
                             Mat &new_model, Score &new_model_score) override
            {
                ...
                for (int iters = 0; iters < lo_inner_max_iterations; iters++)
                {
                    ...
                    if (num_inliers_of_best_model > lo_sample_size)
                    {
                        ...
                        num_estimated_models = estimator->estimateModelNonMinimalSample(lo_sampler->generateUniqueRandomSubset(inliers_of_best_model,
                                                                                                                               num_inliers_of_best_model),
                                                                                        lo_sample_size, lo_models, weights);
                    }
                    else
                    {
                        ...
                        // if it fails -> end Lo.
                        num_estimated_models = estimator->estimateModelNonMinimalSample(inliers_of_best_model, num_inliers_of_best_model, lo_models, weights);
                    }
                    ...
                    if (is_iterative)
                    {
                        ...
                        int virtual_inliers_size = quality->getInliers(new_model, virtual_inliers, lo_threshold);
                        ...
                        for (int iterations = 0; iterations < lo_iter_max_iterations; iterations++)
                        {
                            ...
                            if (virtual_inliers_size > lo_iter_sample_size)
                            {
                                ...
                                num_estimated_models = estimator->estimateModelNonMinimalSample(lo_iter_sampler->generateUniqueRandomSubset(virtual_inliers,
                                                                                                                                            virtual_inliers_size),
                                                                                                lo_iter_sample_size, lo_iter_models, weights);
                            }
                            else
                            {
                                // break if failed, very low probability that it will not fail in next iterations
                                // estimate model with all virtual inliers
                                num_estimated_models = estimator->estimateModelNonMinimalSample(virtual_inliers, virtual_inliers_size, lo_iter_models, weights);
                            }
        ...
        Ptr<InnerIterativeLocalOptimization> InnerIterativeLocalOptimization::create(const Ptr<Estimator> &estimator_, const Ptr<Quality> &quality_,
                                                                                     const Ptr<RandomGenerator> &lo_sampler_, int pts_size,
                                                                                     double threshold_, bool is_iterative_, int lo_iter_sample_size_,
                             ...
                             Mat &new_model, Score &new_model_score) override
            {
                ...
                for (int iterations = 0; iterations < number_of_irwls_iters; iterations++)
                {
                    ...
                    for (int i = 0; i < residual_cnt; i++)
                    {
                        ...
                        int x = (int)round(scale_of_stored_gammas * sqr_residuals[i] / squared_sigma_max_2);
                    ...
                    const int num_est_models = estimator->estimateModelNonMinimalSample(sigma_inliers, std::min(max_lo_sample_size, sigma_inliers_cnt),
        ...
        public:
            LeastSquaresPolishingImpl(const Ptr<Estimator> &estimator_, const Ptr<Quality> &quality_,
                                      int lsq_iterations_) : estimator(estimator_), quality(quality_)
            {
                                         ...
                                         Mat &new_model, Score &out_score) override
            {
                ...
                for (int lsq_iter = 0; lsq_iter < lsq_iterations; lsq_iter++)
                {
                    ...
                    if (fabs(static_cast<double>(out_score.inlier_number) - static_cast<double>(best_model_score.inlier_number)) / best_model_score.inlier_number < 0.05)
                        return true;
