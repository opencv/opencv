...
namespace cv
{
    namespace usac
    {
        ...
        public:
            StandardTerminationCriteriaImpl(double confidence, int points_size_,
                                            int sample_size_, int max_iterations_) : log_confidence(log(1 - confidence)), points_size(points_size_),
     ...
     */
            ...
            {
                const double predicted_iters = log_confidence / log(1 - std::pow(static_cast<double>(inlier_number) / points_size, sample_size));
     ...
     */
        ...
        private:
            ...
            {
                const double predicted_iters = log_eta_0 / log(1 - std::pow(static_cast<double>(inlier_size) / points_size, sample_size));
                return (!std::isinf(predicted_iters) && predicted_iters < MAX_ITERATIONS) ? static_cast<int>(predicted_iters) : MAX_ITERATIONS;
            }
            /*
     * h(i) must hold
     *
     ...
     */
        ...
        public:
            SPRTPNapsacTerminationImpl(const std::vector<SPRT_history> &sprt_histories_,
                ...
                : sprt_termination(sprt_histories_, confidence, points_size_, sample_size_,
                                   max_iterations_),
                  sprt_histories(sprt_histories_),
        ...
        public:
            ProsacTerminationCriteriaImpl(const Ptr<Error> &error_, int points_size_, int sample_size_,
                                          ...
                                          double non_randomness_phi_, double inlier_threshold_) : log_confidence(log(1 - confidence)), beta(beta_), non_randomness_phi(non_randomness_phi_),
     ...
     */
         ...
         */
                ...
                const double predicted_iters = log_confidence / log(1 - std::pow(static_cast<double>(inliers_size) / points_size, sample_size));
