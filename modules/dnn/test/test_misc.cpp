...
{
    Net net = readNet(findDataFile("squeezenet/squeezenet_v1.1.prototxt", false),
                      findDataFile("squeezenet/squeezenet_v1.1.caffemodel", false));
    EXPECT_FALSE(net.empty());
    net = readNet(findDataFile("opencv_face_detector/opencv_face_detector.caffemodel", false),
                  findDataFile("opencv_face_detector/opencv_face_detector.prototxt", false));
    EXPECT_FALSE(net.empty());
    net = readNet(findDataFile("openface/openface_nn4.small2.v1.t7", false));
    EXPECT_FALSE(net.empty());
    net = readNet(findDataFile("yolo/tiny-yolo-voc.cfg", false),
                  findDataFile("yolo/tiny-yolo-voc.weights", false));
    EXPECT_FALSE(net.empty());
    net = readNet(findDataFile("mobilenet/ssd_mobilenet_v1_coco.pbtxt", false),
                  findDataFile("mobilenet/ssd_mobilenet_v1_coco.pb", false));
    EXPECT_FALSE(net.empty());
}
