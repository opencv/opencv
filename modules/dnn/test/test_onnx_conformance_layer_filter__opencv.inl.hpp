...
#if 0
cout << "Filtering is disabled: OpenCV" << endl;
#else


#if 0
// Stats for --gtest_filter=*ONNX_conformance* --gtest_param_filter=*OCV*
[ SKIPSTAT ] 2233 tests skipped
[ SKIPSTAT ] TAG='dnn_skip_opencv_backend' skip 0 tests (112 times in extra skip list)
[ SKIPSTAT ] TAG='dnn_skip_cpu' skip 30 tests
[ SKIPSTAT ] TAG='dnn_skip_ocl' skip 31 tests
[ SKIPSTAT ] TAG='dnn_skip_ocl_fp16' skip 51 tests
[ SKIPSTAT ] TAG='dnn_skip_onnx_conformance' skip 0 tests (2233 times in extra skip list)
[ SKIPSTAT ] TAG='dnn_skip_parser' skip 2121 tests
[==========] 2592 tests from 1 test case ran.
#endif
...
std::string tag_target_skip =
    (target == DNN_TARGET_CPU) ? CV_TEST_TAG_DNN_SKIP_CPU :
    (target == DNN_TARGET_OPENCL) ? CV_TEST_TAG_DNN_SKIP_OPENCL :
    (target == DNN_TARGET_OPENCL_FP16) ? CV_TEST_TAG_DNN_SKIP_OPENCL_FP16 :
    "";
...
if (!filterApplied)
{
    ADD_FAILURE() << "OpenCV backend: unknown test='" << name << "'. Update filter configuration";
}
