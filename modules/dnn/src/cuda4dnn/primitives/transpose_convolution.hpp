...
#ifndef OPENCV_DNN_CUDA4DNN_PRIMITIVES_TRANSPOSE_CONVOLUTION_HPP
#define OPENCV_DNN_CUDA4DNN_PRIMITIVES_TRANSPOSE_CONVOLUTION_HPP
...
namespace cv { namespace dnn { namespace cuda4dnn {
    ...
    public:
            ...
            : stream(std::move(stream_)), cudnnHandle(std::move(handle))
        {
            ...
            if (config.padMode == TransposeConvolutionConfiguration::padding_mode::manual)
            {
            ...
            else if (config.padMode == TransposeConvolutionConfiguration::padding_mode::valid)
            {
                /* nothing to do as the paddings are already preset to zero */
            }
            else if (config.padMode == TransposeConvolutionConfiguration::padding_mode::same)
            {
                /* TensorFlow Logic:
            ...
            csl::TransposeConvolution<T>::params_type params;
...
#endif /* OPENCV_DNN_CUDA4DNN_PRIMITIVES_TRANSPOSE_CONVOLUTION_HPP */
