...
Encoding getEncodingForGPT2FromJSON(const std::string &name, const std::string& json_path) {
...
Tokenizer Tokenizer::load(const std::string& model_dir) {
    ...
    if (model_type == "gpt2") {
        enc = std::make_shared<Encoding>(getEncodingForGPT2FromJSON("gt2", tok_json));
