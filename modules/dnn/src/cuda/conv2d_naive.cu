    ...
    for (int ci = 0; ci < C_in; ++ci) {
        for (int kh = 0; kh < kH; ++kh) {
            ...
            for (int kw = 0; kw < kW; ++kw) {
                ...
                size_t in_idx = ((size_t)n * C_in + ci) * H_in * W_in + (size_t)ih * W_in + iw;
                size_t w_idx  = ((size_t)co * C_in + ci) * kH * kW + (size_t)kh * kW + kw;
                sum += input[in_idx] * weights[w_idx];
            }
        }
    }

    size_t out_idx = ((size_t)n * C_out + co) * H_out * W_out + (size_t)oh * W_out + ow;
    output[out_idx] = sum;
}
    ...
    int padH, int padW)
{
    ...
    conv2d_nchw_fp32_kernel<<<grid, threads>>>(d_input, d_weights, d_bias, d_output,
        N, C_in, H_in, W_in, C_out, kH, kW, strideH, strideW, padH, padW, H_out, W_out);
}
