...
void convBlockMR1(int np, const float* a, const float* b, float *c, const float bias, bool init_c, const float minval,
                  const float maxval, bool ifMinMaxAct, const int convNR);
...
#elif !defined(CV_CPU_OPTIMIZATION_DECLARATIONS_ONLY) && CV_NEON

void convBlock_NEON(int np, const float* a, const float* b, float* c, int ldc, bool init_c, const int convMR, const int convNR)
{
#if CV_NEON_AARCH64
    ...
    if (convMR == 4 && convNR == 12) // ARMv7
    {
        ...
        CV_Error(Error::StsNotImplemented, "Unsupported convMR and/or convNR in convBlock_NEON");
}
