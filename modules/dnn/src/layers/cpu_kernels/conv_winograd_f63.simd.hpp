...
#elif !defined(CV_CPU_OPTIMIZATION_DECLARATIONS_ONLY) && CV_NEON && CV_NEON_AARCH64

void winofunc_accum_f32_NEON(const float* inwptr, const float* wptr, float* outbuf, int Cg, int iblock,
                            const int winoIblock, const int winoKblock, const int winoAtomF32, const int winoNatomF32)
{
...
/*Input transform*/
void winofunc_BtXB_8x8_f32_NEON(const float* inptr, int inpstep,
                          float* outptr, int Cg, const int winoIblock, const int winoAtomF32)
{
...
/*Output transform*/
void winofunc_AtXA_8x8_f32_NEON(const float* inptr, int inpstep,
                          float* bpptr, int bpstep, float* outptr, int outstep,
                          float bias, float minval, float maxval, bool ifMinMaxAct)
{
    float32x4_t x00 = vld1q_f32(inptr), x01 = vld1q_f32(inptr + 4);
