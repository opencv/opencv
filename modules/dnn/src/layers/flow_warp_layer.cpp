...
public:
                                 ...
                                 std::vector<MatShape> &internals) const CV_OVERRIDE
    {
        ...
        const int channels = inputs[0][1];
        const int height = inputs[0][2];
        const int width = inputs[0][3];
        CV_Assert_N(batch == inputs[1][0], inputs[1][1] == 2, height == inputs[1][2], width == inputs[1][3]);

        std::vector<int> outShape;
        outShape.push_back(batch);
        outShape.push_back(channels);
    ...
    void forward(InputArrayOfArrays inputs_arr, OutputArrayOfArrays outputs_arr, OutputArrayOfArrays internals_arr) CV_OVERRIDE
    {
        ...
        const float* image_data = (float*)inputs[0].data; // source image
        const float* flow_data = (float*)inputs[1].data; // source flow
        ...
        for (int n = 0; n < out_n; n++)
        {
            ...
            for (int x = 0; x < out_w; x++)
            {
                ...
                {
                    float fx = flow_data[2 * area * n + y * out_w + x];
                    float fy = flow_data[2 * area * n + area + y * out_w + x];
