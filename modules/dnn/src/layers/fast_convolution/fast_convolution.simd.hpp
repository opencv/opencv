...
static void conv_blockDefualt(int k, const float *a, const float *b,
...
#if CV_NEON
void conv_blockNEON(int k, const float *a, const float *b,
                float *c, int ldc, const float *bias,
                float minval, float maxval, bool ifActiv)
{
...
#else
    CV_LOG_WARNING(NULL, "Runing at unoptimized code. The combination of FAST_CONV_MR and FAST_CONV_NR is not supported in NEON branch.");
    conv_blockDefualt(k, a, b, c, ldc,bias, minval, maxval, ifActiv)
#endif
}
#endif // CV_NEON


void conv_block_SIMD(int k, const float *a, const float *b,
        float *c, int ldc, const float *bias,
        float minval, float maxval, bool ifActiv){
#if CV_SIMD128
...
#endif
#endif
    CV_LOG_WARNING(NULL, "Runing at unoptimized code. The combination of FAST_CONV_MR and FAST_CONV_NR is not supported in SIMD128 branch.");
    conv_blockDefualt(k, a, b, c, ldc,bias, minval, maxval, ifActiv);
