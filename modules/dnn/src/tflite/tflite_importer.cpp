...
void TFLiteImporter::populateNet(Net& dstNet) {
    ...
    for (const auto op : *subgraph->operators()) {
        ...
        if (type == "CUSTOM")
            break;
        // std::cout << type << " " << layerParams.name << std::endl;
        if (type == "CONV_2D") {
        ...
        } else if (type == "DEPTHWISE_CONV_2D") {
            layerParams.type = "Convolution";
        ...
        } else if (type == "MAX_POOL_2D" || type == "AVERAGE_POOL_2D") {
            ...
            else if (type == "AVERAGE_POOL_2D")
                layerParams.set("pool", "ave");
            else
                CV_Error(Error::StsNotImplemented, "Pool type selection for " + type);
        } else if (type == "PAD") {
