...
class MetalFlattenImpl final : public metal::Operation
{
public:
    MetalFlattenImpl(
    ...
    void forward(metal::Context& context) CV_OVERRIDE
    {
        if (!pipeline_)
        {
            pipeline_ = metal::Device::instance()->pipelineState("kernel_flatten_f32");
        }
...
static std::unique_ptr<metal::Operation> makeFlattenOperation(
        const std::vector<Ptr<BackendWrapper> >& inputs,
                               const std::vector<Ptr<BackendWrapper> >& outputs)
{
    return std::unique_ptr<metal::Operation>(new MetalFlattenImpl(
...
Ptr<BackendNode> FlattenOp::create(const std::vector<Ptr<BackendWrapper> >& inputs,
                                const std::vector<Ptr<BackendWrapper> >& outputs)
{
    return makeMetalBackendNode(makeFlattenOperation(inputs, outputs));
}
