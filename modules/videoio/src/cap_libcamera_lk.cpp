...
namespace cv
{
    ...
    LibcameraApp::LibcameraApp(std::unique_ptr<Options> opts)
        : options_(std::move(opts)), controls_(controls::controls), capture_instance_(nullptr)
    {
        if (!options_)
    ...
    void LibcameraApp::requestComplete(Request *request)
    {
        ...
        if (capture_instance_) {
            capture_instance_->onRequestComplete(payload);
        } else {
    ...
    protected:
        ...
        // static constexpr size_t PREFERRED_QUEUE_SIZE = 1;  // 理想队列大小
