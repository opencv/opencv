...
*/
// require Windows 8 for some of the formats defined otherwise could baseline on
// lower version
...
typedef HRESULT(WINAPI *FN_MFCreateDXGIDeviceManager)(
    UINT *resetToken, IMFDXGIDeviceManager **ppDeviceManager);
...
static void init_MFCreateDXGIDeviceManager() {
  ...
  if (h) {
    pMFCreateDXGIDeviceManager = (FN_MFCreateDXGIDeviceManager)GetProcAddress(
        h, "MFCreateDXGIDeviceManager");
  }
...
public:
  ComPtr() {}
  ComPtr(T *lp) { p = lp; }
  ComPtr(_In_ const ComPtr<T> &lp) { p = lp.p; }
  virtual ~ComPtr() {}
  ...
  operator bool() { return p.operator!=(NULL); }
  ...
  template <typename U> HRESULT As(_Out_ ComPtr<U> &lp) const {
    lp.Release();
...
// see
// https://learn.microsoft.com/en-us/windows/win32/medfound/video-subtype-guids#luminance-and-depth-formats
DEFINE_MEDIATYPE_GUID(OCV_MFVideoFormat_D16, 80);

//==================================================================================================

// Structure for collecting info about types of video which are supported by
...
struct MediaType {
  ...
  MediaType(IMFMediaType *pType = 0)
      : Type(pType), width(0), height(0), stride(0), isFixedSize(true),
        frameRateNum(1), frameRateDenom(1), aspectRatioNum(1),
        aspectRatioDenom(1), sampleSize(0), interlaceMode(0), bit_per_sample(0),
        nChannels(0), nAvgBytesPerSec(0), nSamplesPerSec(0),
    ...
    if (pType) {
      ...
      if (majorType == MFMediaType_Audio) {
        ...
        pType->GetUINT32(MF_MT_DEFAULT_STRIDE,
                         (UINT32 *)&stride); // value is stored as UINT32 but
                                             // should be casted to INT3)
        pType->GetUINT32(MF_MT_FIXED_SIZE_SAMPLES, &isFixedSize);
        MFGetAttributeRatio(pType, MF_MT_FRAME_RATE, &frameRateNum,
                            &frameRateDenom);
        MFGetAttributeRatio(pType, MF_MT_PIXEL_ASPECT_RATIO, &aspectRatioNum,
                            &aspectRatioDenom);
  ...
  _ComPtr<IMFMediaType> createMediaType_Video() const {
    ...
    if (frameRateNum != 0 || frameRateDenom != 0)
      MFSetAttributeRatio(res.Get(), MF_MT_FRAME_RATE, frameRateNum,
                          frameRateDenom);
    if (aspectRatioNum != 0 || aspectRatioDenom != 0)
      MFSetAttributeRatio(res.Get(), MF_MT_PIXEL_ASPECT_RATIO, aspectRatioNum,
                          aspectRatioDenom);
    if (sampleSize > 0)
  ...
  double getFramerate() const {
    return frameRateDenom != 0
               ? ((double)frameRateNum) / ((double)frameRateDenom)
               : 0;
  }
  ...
  bool VideoIsBetterThan(const MediaType &other, const MediaType &ref) const {
    ...
    if (thisDiff == otherDiff) {
      ...
      if (width == other.width && height == other.height) {
        ...
        const double otherRateDiff =
            absDiff(other.getFramerate(), ref.getFramerate());
  ...
  bool VideoIsAvailable() const { return (subType != OCV_MFVideoFormat_D16); }
};

void printFormat(std::ostream &out, const GUID &fmt) {
#define PRNT(FMT) else if (fmt == FMT) out << #FMT;
  if (fmt == MFVideoFormat_Base)
    out << "Base";
...
public:
  ...
  SourceReaderCB()
      : m_nRefCount(0), m_hEvent(CreateEvent(NULL, FALSE, FALSE, NULL)),
        m_bEOS(FALSE), m_hrStatus(S_OK), m_reader(NULL), m_dwStreamIndex(0) {}
...
#pragma warning(pop)
  ...
  STDMETHODIMP OnReadSample(HRESULT hrStatus, DWORD dwStreamIndex,
...
#else
          // this branch reduces latency if we drop frames due to slow
          // processing. avoid fetching of already outdated frames from the
          // queue's front.
          CV_LOG_DEBUG(NULL,
                       "videoio(MSMF): drop previous frames (not processed): "
                           << m_capturedFrames.size());
          std::queue<CapturedFrameInfo>().swap(
              m_capturedFrames); // similar to missing m_capturedFrames.clean();
#endif
        }
        m_capturedFrames.emplace(CapturedFrameInfo{
            llTimestamp, _ComPtr<IMFSample>(pSample), hrStatus});
      }
    } else {
      CV_LOG_WARNING(
          NULL, "videoio(MSMF): OnReadSample() is called with error status: "
                    << hrStatus);
    }
    ...
    if (FAILED(hr = m_reader->ReadSample(dwStreamIndex, 0, NULL, NULL, NULL,
                                         NULL))) {
      CV_LOG_WARNING(
          NULL,
  ...
  STDMETHODIMP OnEvent(DWORD, IMFMediaEvent *) CV_OVERRIDE { return S_OK; }
  STDMETHODIMP OnFlush(DWORD) CV_OVERRIDE { return S_OK; }

  HRESULT Wait(DWORD dwMilliseconds, _ComPtr<IMFSample> &mediaSample,
               LONGLONG &sampleTimestamp, BOOL &pbEOS) {
    pbEOS = FALSE;
    ...
    for (;;) {
      {
        ...
        if (!m_capturedFrames.empty()) {
          ...
          CapturedFrameInfo frameInfo = m_capturedFrames.front();
          m_capturedFrames.pop();
          CV_LOG_DEBUG(NULL, "videoio(MSMF): handle frame at "
                                 << frameInfo.timestamp);
          mediaSample = frameInfo.sample;
          CV_Assert(mediaSample);
          sampleTimestamp = frameInfo.timestamp;
          ResetEvent(
              m_hEvent); // event is auto-reset, but we need this forced reset
                         // due time gap between wait() and mutex hold.
...
private:
  ...
  virtual ~SourceReaderCB() { CV_LOG_INFO(NULL, "terminating async callback"); }
...
// Enumerate and store supported formats and finds format which is most similar
...
public:
  struct MediaID {
    ...
    void nextMedia() { media++; }
    bool operator<(const MediaID &other) const {
      return (stream < other.stream) ||
             (stream == other.stream && media < other.media);
    }
  };
    ...
    for (; i != formats.end(); ++i) {
      if (i->second.majorType == MFMediaType_Video) {
        if (best.second.isEmpty() ||
            (i->second.VideoIsBetterThan(best.second, newType) &&
             i->second.VideoIsAvailable())) {
          best = *i;
        }
      }
    }
  ...
  std::pair<MediaID, MediaType>
  findAudioFormatByStream(const DWORD StreamIndex) {
    std::pair<MediaID, MediaType> best;
    std::map<MediaID, MediaType>::const_iterator i = formats.begin();
    for (; i != formats.end(); ++i) {
      if (i->second.majorType == MFMediaType_Audio) {
...
public:
  ...
  _ComPtr<IMFMediaSource> activateSource(UINT32 index) {
    ...
    if (count == 0 || index >= count ||
        FAILED(devices[index]->ActivateObject(__uuidof(IMFMediaSource),
                                              (void **)&result))) {
      CV_LOG_DEBUG(NULL,
                   "Failed to activate media source (device " << index << ")");
    }
    return result;
  }
...
public:
  typedef enum { MODE_SW = 0, MODE_HW = 1 } MSMFCapture_Mode;
  CvCapture_MSMF();
  ...
  virtual bool open(const cv::String &, const Ptr<IStreamReader> &,
...
protected:
  ...
  template <typename CtrlT> bool readComplexPropery(long prop, long &val) const;
  ...
  _ComPtr<IMFSourceReaderCallback>
      readCallback; // non-NULL for "live" streams (camera capture)
  ...
  LONGLONG numberOfAdditionalAudioBytes; // the number of additional bytes
                                         // required to align the audio chunk
...
CvCapture_MSMF::CvCapture_MSMF()
    : MF(Media_Foundation::getInstance()), filename(""), camid(-1),
      captureMode(MODE_SW), va_type(VIDEO_ACCELERATION_NONE), hwDeviceIndex(-1),
#ifdef HAVE_MSMF_DXVA
      D3DDev(NULL), D3DMgr(NULL),
#endif
      videoFileSource(NULL), readCallback(NULL), impendingVideoSample(NULL),
      usedVideoSample(NULL), dwVideoStreamIndex(0), dwAudioStreamIndex(0),
      device_status(false), videoStream(0), audioStream(-1), vEOS(false),
      aEOS(false), audioBaseIndex(1), outputVideoFormat(CV_CAP_MODE_BGR),
      outputAudioFormat(CV_16S), audioSamplesPerSecond(0), convertFormat(true),
      duration(0), frameStep(0), nFrame(0), impendingVideoSampleTime(0),
      usedVideoSampleTime(0), videoStartOffset(-1), videoSampleDuration(0),
      requiredAudioTime(0), audioSampleTime(0), audioStartOffset(-1),
      audioSampleDuration(0), audioTime(0), chunkLengthOfBytes(0),
      givenAudioTime(0), numberOfAdditionalAudioBytes(0),
      bufferedAudioDuration(0), audioSamplePos(0), numberOfAudioStreams(0),
      isOpen(false), grabIsDone(false), syncLastFrame(true), lastFrame(false) {
}
...
bool CvCapture_MSMF::initStream(DWORD streamID, const MediaType mt) {
  ...
  HRESULT hr =
      videoFileSource->SetCurrentMediaType(streamID, NULL, mediaTypesOut.Get());
  if (hr == MF_E_TOPO_CODEC_NOT_FOUND) {
    CV_LOG_WARNING(NULL, "Failed to set mediaType (stream "
                             << streamID << ", " << mt << "(codec not found)");
    return false;
...
_ComPtr<IMFAttributes> CvCapture_MSMF::getDefaultSourceConfig(UINT32 num) {
  CV_Assert(num > 0);
  const bool OPENCV_VIDEOIO_MSMF_ENABLE_HW_TRANSFORMS =
      utils::getConfigurationParameterBool(
          "OPENCV_VIDEOIO_MSMF_ENABLE_HW_TRANSFORMS", true);
  _ComPtr<IMFAttributes> res;
  if (FAILED(MFCreateAttributes(&res, num)) ||
      FAILED(res->SetUINT32(MF_READWRITE_ENABLE_HARDWARE_TRANSFORMS,
      ...
      FAILED(res->SetUINT32(MF_SOURCE_READER_ENABLE_ADVANCED_VIDEO_PROCESSING,
                            true))) {
    CV_Error(cv::Error::StsError, "Failed to create attributes");
  }
#ifdef HAVE_MSMF_DXVA
...
bool CvCapture_MSMF::configureHW(bool enable) {
  ...
  if (enable) {
    ...
    if (SUCCEEDED(D3D11CreateDevice(pAdapter.Get(), driverType, NULL,
                                    D3D11_CREATE_DEVICE_BGRA_SUPPORT |
                                        D3D11_CREATE_DEVICE_VIDEO_SUPPORT,
                                    levels, sizeof(levels) / sizeof(*levels),
                                    D3D11_SDK_VERSION, &D3DDev, NULL, NULL))) {
      ...
      if (SUCCEEDED(D3DDev->QueryInterface(IID_PPV_ARGS(&D3DDevMT)))) {
        ...
        if (SUCCEEDED(pMFCreateDXGIDeviceManager(&mgrRToken, &D3DMgr))) {
          if (SUCCEEDED(D3DMgr->ResetDevice(D3DDev.Get(), mgrRToken))) {
            ...
            if (SUCCEEDED(D3DDev->QueryInterface(
              ...
              if (SUCCEEDED(dxgiDevice->GetAdapter(&adapter))) {
                ...
                if (SUCCEEDED(adapter->GetDesc(&desc))) {
                  ...
                  CV_LOG_INFO(
                      NULL,
                      "MSMF: Using D3D11 video acceleration on GPU device: "
                          << conv.to_bytes(name));
                }
              }
            }
            // Reopen if needed
            return reopen
                       ? (prevcam >= 0 ? open(prevcam, NULL)
                                       : open(prevfile.c_str(), nullptr, NULL))
                       : true;
          }
          D3DMgr.Release();
        }
    ...
    if (D3DDev)
      D3DDev.Release();
    captureMode = MODE_SW;
    return reopen ? (prevcam >= 0 ? open(prevcam, NULL)
                                  : open(prevfile.c_str(), nullptr, NULL))
                  : true;
  }
#else
...
bool CvCapture_MSMF::configureHW(const VideoCaptureParameters &params) {
  ...
  if (va_type != VIDEO_ACCELERATION_NONE && va_type != VIDEO_ACCELERATION_ANY) {
    CV_LOG_INFO(NULL,
                "VIDEOIO/MSMF: MSMF backend is build without DXVA acceleration "
                "support. Can't handle CAP_PROP_HW_ACCELERATION parameter: "
                    << va_type);
  }
#endif
  return configureHW(va_type == VIDEO_ACCELERATION_D3D11 ||
                     va_type == VIDEO_ACCELERATION_ANY);
}
...
bool CvCapture_MSMF::configureAudioOutput(MediaType newType) {
  ...
  newFormat.nSamplesPerSec =
      (audioSamplesPerSecond == 0) ? 44100 : audioSamplesPerSecond;
...
bool CvCapture_MSMF::configureVideoOutput(MediaType newType,
                                          cv::uint32_t outFormat) {
  FormatStorage formats;
  formats.read(videoFileSource.Get());
  std::pair<FormatStorage::MediaID, MediaType> bestMatch =
      formats.findBestVideoFormat(newType);
  ...
  if (convertFormat) {
    ...
    case CV_CAP_MODE_RGB:
      newFormat.subType =
          captureMode == MODE_HW ? MFVideoFormat_RGB32 : MFVideoFormat_RGB24;
    ...
    if (nativeFormat.subType ==
        MFVideoFormat_MP43) // Unable to estimate FPS for MP43
      newFormat.frameRateNum = 0;
  }
  // we select native format first and then our requested format (related issue
  // #12822)
...
bool CvCapture_MSMF::configureOutput() {
  if (FAILED(videoFileSource->SetStreamSelection(
  ...
  if (videoStream != -1)
    tmp = (!device_status)
              ? configureVideoOutput(MediaType(), outputVideoFormat)
              : configureVideoOutput(MediaType::createDefault_Video(),
                                     outputVideoFormat);
  if (audioStream != -1)
    tmp &= (!device_status)
               ? configureAudioOutput(MediaType())
               : configureAudioOutput(MediaType::createDefault_Audio());
  return tmp;
}
...
bool CvCapture_MSMF::open(int index, const cv::VideoCaptureParameters *params) {
  ...
  if (videoStream != -1 && audioStream != -1 ||
    ...
    CV_LOG_DEBUG(NULL, "Only one of the properties CAP_PROP_AUDIO_STREAM "
                           << audioStream << " and " << CAP_PROP_VIDEO_STREAM
  ...
  if (count == 0 || static_cast<UINT32>(index) > count) {
    CV_LOG_DEBUG(NULL, "Device " << index << " not found (total " << count
                                 << " devices)");
    return false;
  }
  ...
  if (!src.Get() || FAILED(MFCreateSourceReaderFromMediaSource(
                        src.Get(), attr.Get(), &videoFileSource))) {
...
bool CvCapture_MSMF::open(const cv::String &_filename,
                          const Ptr<IStreamReader> &stream,
                          const cv::VideoCaptureParameters *params) {
  close();
  ...
  if (!_filename.empty()) {
    ...
    MultiByteToWideChar(CP_ACP, 0, _filename.c_str(), -1,
                        unicodeFileName.data(), (int)_filename.length() + 1);
    succeeded = SUCCEEDED(MFCreateSourceReaderFromURL(
        unicodeFileName.data(), attr.Get(), &videoFileSource));
  } else if (stream) {
    ...
    IStream *s = SHCreateMemStream(reinterpret_cast<const BYTE *>(data.data()),
    ...
    if (!SUCCEEDED(MFStartup(MF_VERSION)))
      return false;
    succeeded = SUCCEEDED(MFCreateSourceReaderFromByteStream(
        byteStream.Get(), attr.Get(), &videoFileSource));
  }
  ...
  if (succeeded) {
    ...
    if (configureOutput()) {
      ...
      HRESULT hr;
      if (SUCCEEDED(
              hr = videoFileSource->GetPresentationAttribute(
...
bool CvCapture_MSMF::openFinalize_(const VideoCaptureParameters *params) {
  ...
  VideoAccelerationType actual_va_type = (captureMode == MODE_HW)
                                             ? VIDEO_ACCELERATION_D3D11
                                             : VIDEO_ACCELERATION_NONE;
  if (va_type != VIDEO_ACCELERATION_NONE && va_type != VIDEO_ACCELERATION_ANY) {
    if (va_type != actual_va_type) {
      CV_LOG_ERROR(NULL, "VIDEOIO/MSMF: Can't select requested video "
                         "acceleration through CAP_PROP_HW_ACCELERATION: "
                             << va_type << " (actual is " << actual_va_type
                             << "). Bailout");
      return false;
    }
  } else {
...
bool CvCapture_MSMF::configureStreams(
    const cv::VideoCaptureParameters &params) {
  if (params.has(CAP_PROP_VIDEO_STREAM)) {
    ...
    if (value == -1 || value == 0)
      ...
      CV_LOG_ERROR(NULL, "VIDEOIO/MSMF: CAP_PROP_VIDEO_STREAM parameter value "
  ...
  if (params.has(CAP_PROP_AUDIO_STREAM)) {
    ...
    if (value == -1 || value > -1)
      ...
      CV_LOG_ERROR(NULL, "VIDEOIO/MSMF: CAP_PROP_AUDIO_STREAM parameter value "
                         "is invalid/unsupported: "
                             << value);
      return false;
    }
...
bool CvCapture_MSMF::setAudioProperties(
    const cv::VideoCaptureParameters &params) {
  if (params.has(CAP_PROP_AUDIO_DATA_DEPTH)) {
    int value = static_cast<int>(params.get<double>(CAP_PROP_AUDIO_DATA_DEPTH));
    if (value != CV_8S && value != CV_16S && value != CV_32S &&
        value != CV_32F) {
      CV_LOG_ERROR(NULL, "VIDEOIO/MSMF: CAP_PROP_AUDIO_DATA_DEPTH parameter "
                         "value is invalid/unsupported: "
                             << value);
      return false;
    } else {
  ...
  if (params.has(CAP_PROP_AUDIO_SAMPLES_PER_SECOND)) {
    int value =
        static_cast<int>(params.get<double>(CAP_PROP_AUDIO_SAMPLES_PER_SECOND));
    if (value < 0) {
      CV_LOG_ERROR(NULL, "VIDEOIO/MSMF: CAP_PROP_AUDIO_SAMPLES_PER_SECOND "
  ...
  if (params.has(CAP_PROP_AUDIO_SYNCHRONIZE)) {
    int value =
        static_cast<UINT32>(params.get<double>(CAP_PROP_AUDIO_SYNCHRONIZE));
...
bool CvCapture_MSMF::checkAudioProperties() {
  if (audioSamplesPerSecond != 0) {
    ...
    HRESULT hr =
        videoFileSource->GetCurrentMediaType(dwAudioStreamIndex, &type);
    if (SUCCEEDED(hr)) {
      type->GetUINT32(MF_MT_AUDIO_SAMPLES_PER_SECOND,
                      &actualAudioSamplesPerSecond);
      if (actualAudioSamplesPerSecond != audioSamplesPerSecond) {
        CV_LOG_ERROR(
            NULL,
            "VIDEOIO/MSMF: CAP_PROP_AUDIO_SAMPLES_PER_SECOND parameter value "
            "is invalid/unsupported: "
  ...
  while (!stopFlag) {
    for (;;) {
      ...
      if (streamIndex != dwVideoStreamIndex)
        break;
      if (flags &
          (MF_SOURCE_READERF_ERROR | MF_SOURCE_READERF_ALLEFFECTSREMOVED |
    ...
    if (SUCCEEDED(hr)) {
      if (streamIndex != dwVideoStreamIndex) {
        ...
        CV_LOG_DEBUG(NULL, "videoio(MSMF): got video frame with timestamp="
                               << impendingVideoSampleTime);
        if (audioStream != -1) {
          if (!usedVideoSample) {
            usedVideoSample.swap(impendingVideoSample);
            std::swap(usedVideoSampleTime, impendingVideoSampleTime);
          ...
          if (impendingVideoSample) {
            nFrame++;
            videoSampleDuration =
                impendingVideoSampleTime - usedVideoSampleTime;
...
bool CvCapture_MSMF::configureAudioFrame() {
  if (!audioSamples.empty() || !bufferAudioData.empty() && aEOS) {
    const int bytesPerSample =
        (captureAudioFormat.bit_per_sample / 8) * captureAudioFormat.nChannels;
    ...
    chunkLengthOfBytes =
        (videoStream != -1)
            ? (LONGLONG)((requiredAudioTime *
                          captureAudioFormat.nSamplesPerSec * bytesPerSample) /
                         1e7)
            : cursize;

    if ((videoStream != -1) && (chunkLengthOfBytes % bytesPerSample != 0)) {
      if ((double)audioSamplePos / captureAudioFormat.nSamplesPerSec +
              audioStartOffset * 1e-7 - usedVideoSampleTime * 1e-7 >=
          0)
        chunkLengthOfBytes -= numberOfAdditionalAudioBytes;

      numberOfAdditionalAudioBytes =
          bytesPerSample - (chunkLengthOfBytes % bytesPerSample);
      chunkLengthOfBytes += numberOfAdditionalAudioBytes;
    }
    ...
    CV_Check((double)chunkLengthOfBytes,
             chunkLengthOfBytes >= INT_MIN && chunkLengthOfBytes <= INT_MAX,
             "MSMF: The chunkLengthOfBytes is out of the allowed range");

    if (chunkLengthOfBytes > static_cast<LONGLONG>(bufferAudioData.size())) {
      CV_LOG_WARNING(NULL,
                     "MSMF: chunkLengthOfBytes exceeds buffer size, clamping");
      chunkLengthOfBytes = bufferAudioData.size();
    }

    copy(bufferAudioData.begin(),
         bufferAudioData.begin() + (int)chunkLengthOfBytes,
         std::back_inserter(audioDataInUse));

    bufferAudioData.erase(bufferAudioData.begin(),
                          bufferAudioData.begin() + (int)chunkLengthOfBytes);
    ...
    if (audioFrame.empty()) {
      ...
      switch (outputAudioFormat) {
      case CV_8S:
        numSamples = (int)chunkLengthOfBytes / bytesPerSample;
        cv::Mat(numSamples, captureAudioFormat.nChannels, CV_8S,
                audioDataInUse.data())
            .copyTo(audioFrame);
        break;
      case CV_16S:
        numSamples = (int)chunkLengthOfBytes / bytesPerSample;
        cv::Mat(numSamples, captureAudioFormat.nChannels, CV_16S,
                audioDataInUse.data())
            .copyTo(audioFrame);
        break;
      case CV_32S:
        numSamples = (int)chunkLengthOfBytes / bytesPerSample;
        cv::Mat(numSamples, captureAudioFormat.nChannels, CV_32S,
                audioDataInUse.data())
            .copyTo(audioFrame);
        break;
      case CV_32F:
        numSamples = (int)chunkLengthOfBytes / bytesPerSample;
        cv::Mat(numSamples, captureAudioFormat.nChannels, CV_32F,
                audioDataInUse.data())
            .copyTo(audioFrame);
        break;
      default:
        CV_LOG_WARNING(NULL, "MSMF: Unsupported audio output format");
        break;
      }
    }
  ...
  while ((!vEOS) ? audioTime <= requiredAudioTime : !aEOS) {
    ...
    if (SUCCEEDED(hr)) {
      if (streamIndex != dwAudioStreamIndex) {
        ...
        if (videoStream == -1)
          audioSamplePos +=
              chunkLengthOfBytes / ((captureAudioFormat.bit_per_sample / 8) *
                                    captureAudioFormat.nChannels);
        ...
        CV_LOG_DEBUG(NULL, "videoio(MSMF): got audio frame with timestamp="
                               << audioSampleTime
                               << "  duration=" << audioSampleDuration);
        audioTime +=
            (LONGLONG)(audioSampleDuration + bufferedAudioDuration * 1e7);
...
bool CvCapture_MSMF::grabFrame() {
  ...
  if (grabIsDone) {
    grabIsDone = false;
    CV_LOG_DEBUG(NULL, "videoio(MSMF): return pre-grabbed frame "
                           << usedVideoSampleTime);
    return true;
  }
  ...
  if (readCallback) // async "live" capture mode
  {
    ...
    if (!reader->m_reader) {
      ...
      if (FAILED(hr = videoFileSource->ReadSample(dwStreamIndex, 0, NULL, NULL,
                                                  NULL, NULL))) {
        CV_LOG_ERROR(NULL, "videoio(MSMF): can't grab frame - initial async "
                           "ReadSample() call failed: "
                               << hr);
    ...
    if (FAILED(hr = reader->Wait(videoStream == -1 ? INFINITE : 10000,
                                 (videoStream != -1) ? usedVideoSample
                                                     : audioSamples[0],
                                 timestamp, bEOS))) // 10 sec
    {
    ...
    if (audioStream != -1) {
      ...
      const int bytesPerSample = (captureAudioFormat.bit_per_sample / 8) *
                                 captureAudioFormat.nChannels;
      bufferedAudioDuration =
          (double)(bufferAudioData.size() / bytesPerSample) /
          captureAudioFormat.nSamplesPerSec;
      audioFrame.release();
      if (!aEOS)
  ...
  do {
    ...
    if (convertFormat) {
      if (lock2d || (unsigned int)cursize == captureVideoFormat.sampleSize) {
        ...
        case CV_CAP_MODE_YUYV:
          cv::Mat(captureVideoFormat.height, captureVideoFormat.width, CV_8UC2,
                  ptr, pitch)
              .copyTo(frame);
          break;
        case CV_CAP_MODE_BGR:
          if (captureMode == MODE_HW)
            cv::cvtColor(cv::Mat(captureVideoFormat.height,
                                 captureVideoFormat.width, CV_8UC4, ptr, pitch),
                         frame, cv::COLOR_BGRA2BGR);
          else
            cv::Mat(captureVideoFormat.height, captureVideoFormat.width,
                    CV_8UC3, ptr, pitch)
                .copyTo(frame);
          break;
        case CV_CAP_MODE_RGB:
          if (captureMode == MODE_HW)
            ...
            cv::cvtColor(cv::Mat(captureVideoFormat.height,
                                 captureVideoFormat.width, CV_8UC3, ptr, pitch),
                         frame, cv::COLOR_BGR2RGB);
          break;
        case CV_CAP_MODE_GRAY:
          cv::Mat(captureVideoFormat.height, captureVideoFormat.width, CV_8UC1,
                  ptr, pitch)
              .copyTo(frame);
          break;
        default:
...
bool CvCapture_MSMF::setTime(double time, bool rough) {
  ...
  if (videoStream != -1 && audioStream != -1)
    if (time != 0)
      return false;
  PROPVARIANT var;
  if (SUCCEEDED(videoFileSource->GetPresentationAttribute(
          (DWORD)MF_SOURCE_READER_MEDIASOURCE,
          MF_SOURCE_READER_MEDIASOURCE_CHARACTERISTICS, &var)) &&
      var.vt == VT_UI4 && var.ulVal & MFMEDIASOURCE_CAN_SEEK) {
    usedVideoSample.Release();
    bool useGrabbing =
        time > 0 && !rough && !(var.ulVal & MFMEDIASOURCE_HAS_SLOW_SEEK);
    PropVariantClear(&var);
    ...
    if (resOK && useGrabbing) {
      ...
      do {
        resOK = grabFrame();
        usedVideoSample.Release();
      } while (resOK && usedVideoSampleTime < timeborder);
    }
    return resOK;
  }
  return false;
...
bool CvCapture_MSMF::setTime(int numberFrame) {
  ...
  if (SUCCEEDED(videoFileSource->GetPresentationAttribute(
    ...
    bool resOK = SUCCEEDED(videoFileSource->SetCurrentPosition(GUID_NULL, var));
    PropVariantClear(&var);
    while (resOK && nFrame < numberFrame) {
      resOK = grabFrame();
      usedVideoSample.Release();
    };
    return resOK;
...
bool CvCapture_MSMF::readComplexPropery(long prop, long &val) const {
  _ComPtr<CtrlT> ctrl;
  if (FAILED(videoFileSource->GetServiceForStream(
  ...
  if (FAILED(ctrl->GetRange(prop, &minVal, &maxVal, &stepVal, &paramVal,
                            &paramFlag))) {
...
double CvCapture_MSMF::getProperty(int property_id) const {
  ...
  if (isOpen)
    ...
    case CAP_PROP_FRAME_COUNT:
      if (duration != 0)
        return floor(
            ((double)duration / 1e7) * captureVideoFormat.getFramerate() + 0.5);
      else
        break;
    ...
    case CAP_PROP_BACKLIGHT:
      if (readComplexPropery<IAMVideoProcAmp>(
              VideoProcAmp_BacklightCompensation, cVal))
        return cVal;
      break;
...
bool CvCapture_MSMF::setProperty(int property_id, double value) {
  ...
  if (isOpen)
    ...
    case CAP_PROP_BRIGHTNESS:
      return writeComplexProperty<IAMVideoProcAmp>(
          VideoProcAmp_Brightness, value, VideoProcAmp_Flags_Manual);
    case CAP_PROP_CONTRAST:
      return writeComplexProperty<IAMVideoProcAmp>(VideoProcAmp_Contrast, value,
                                                   VideoProcAmp_Flags_Manual);
    case CAP_PROP_SATURATION:
      return writeComplexProperty<IAMVideoProcAmp>(
          VideoProcAmp_Saturation, value, VideoProcAmp_Flags_Manual);
    case CAP_PROP_HUE:
      return writeComplexProperty<IAMVideoProcAmp>(VideoProcAmp_Hue, value,
                                                   VideoProcAmp_Flags_Manual);
    case CAP_PROP_GAIN:
      return writeComplexProperty<IAMVideoProcAmp>(VideoProcAmp_Gain, value,
                                                   VideoProcAmp_Flags_Manual);
    case CAP_PROP_SHARPNESS:
      return writeComplexProperty<IAMVideoProcAmp>(
          VideoProcAmp_Sharpness, value, VideoProcAmp_Flags_Manual);
    case CAP_PROP_GAMMA:
      return writeComplexProperty<IAMVideoProcAmp>(VideoProcAmp_Gamma, value,
                                                   VideoProcAmp_Flags_Manual);
    case CAP_PROP_BACKLIGHT:
      return writeComplexProperty<IAMVideoProcAmp>(
          VideoProcAmp_BacklightCompensation, value, VideoProcAmp_Flags_Manual);
    case CAP_PROP_MONOCHROME:
      return writeComplexProperty<IAMVideoProcAmp>(
          VideoProcAmp_ColorEnable, value, VideoProcAmp_Flags_Manual);
    case CAP_PROP_TEMPERATURE:
      return writeComplexProperty<IAMVideoProcAmp>(
          VideoProcAmp_WhiteBalance, value, VideoProcAmp_Flags_Manual);
    case CAP_PROP_PAN:
      return writeComplexProperty<IAMCameraControl>(CameraControl_Pan, value,
                                                    CameraControl_Flags_Manual);
    case CAP_PROP_TILT:
      return writeComplexProperty<IAMCameraControl>(CameraControl_Tilt, value,
                                                    CameraControl_Flags_Manual);
    case CAP_PROP_ROLL:
      return writeComplexProperty<IAMCameraControl>(CameraControl_Roll, value,
                                                    CameraControl_Flags_Manual);
    case CAP_PROP_IRIS:
      return writeComplexProperty<IAMCameraControl>(CameraControl_Iris, value,
                                                    CameraControl_Flags_Manual);
    case CAP_PROP_EXPOSURE:
      return writeComplexProperty<IAMCameraControl>(
          CameraControl_Exposure, value, CameraControl_Flags_Manual);
    case CAP_PROP_AUTO_EXPOSURE:
      return writeComplexProperty<IAMCameraControl>(
          CameraControl_Exposure, value,
          value != 0 ? VideoProcAmp_Flags_Auto : VideoProcAmp_Flags_Manual);
    case CAP_PROP_ZOOM:
      return writeComplexProperty<IAMCameraControl>(CameraControl_Zoom, value,
                                                    CameraControl_Flags_Manual);
    case CAP_PROP_FOCUS:
      return writeComplexProperty<IAMCameraControl>(CameraControl_Focus, value,
                                                    CameraControl_Flags_Manual);
    case CAP_PROP_AUTOFOCUS:
      return writeComplexProperty<IAMCameraControl>(
          CameraControl_Focus, value,
...
cv::Ptr<cv::IVideoCapture>
cv::cvCreateCapture_MSMF(int index, const cv::VideoCaptureParameters &params) {
...
cv::Ptr<cv::IVideoCapture>
cv::cvCreateCapture_MSMF(const cv::String &filename,
                         const cv::VideoCaptureParameters &params) {
  cv::Ptr<CvCapture_MSMF> capture = cv::makePtr<CvCapture_MSMF>();
  if (capture) {
...
cv::Ptr<cv::IVideoCapture>
cv::cvCreateCapture_MSMF(const Ptr<IStreamReader> &stream,
                         const cv::VideoCaptureParameters &params) {
  cv::Ptr<CvCapture_MSMF> capture = cv::makePtr<CvCapture_MSMF>();
  if (capture) {
...
public:
  ...
  virtual bool open(const cv::String &filename, int fourcc, double fps,
                    cv::Size frameSize,
...
CvVideoWriter_MSMF::CvVideoWriter_MSMF()
    : MF(Media_Foundation::getInstance()), va_type(VIDEO_ACCELERATION_NONE),
      va_device(-1), videoWidth(0), videoHeight(0), fps(0), bitRate(0),
      frameSize(0), encodingFormat(), inputFormat(), streamIndex(0),
      initiated(false), rtStart(0), rtDuration(0) {}
...
bool CvVideoWriter_MSMF::open(const cv::String &filename, int fourcc,
                              double _fps, cv::Size _frameSize,
                              const cv::VideoWriterParameters &params) {
  if (initiated)
    close();
  ...
  if (params.has(VIDEOWRITER_PROP_HW_ACCELERATION)) {
    va_type =
        params.get<VideoAccelerationType>(VIDEOWRITER_PROP_HW_ACCELERATION);
    if (va_type != VIDEO_ACCELERATION_NONE &&
        va_type != VIDEO_ACCELERATION_ANY) {
      CV_LOG_ERROR(NULL, "VIDEOIO/MSMF: MSMF backend doesn't support writer "
                         "acceleration support. Can't handle "
                         "VIDEOWRITER_PROP_HW_ACCELERATION parameter. Bailout");
      return false;
    }
  }
  if (params.has(VIDEOWRITER_PROP_HW_DEVICE)) {
    ...
    if (va_type == VIDEO_ACCELERATION_NONE && va_device != -1) {
      CV_LOG_ERROR(NULL,
                   "VIDEOIO/MSMF: Invalid usage of VIDEOWRITER_PROP_HW_DEVICE "
                   "without requested H/W acceleration. Bailout");
      return false;
    }
    if (va_type == VIDEO_ACCELERATION_ANY && va_device != -1) {
      CV_LOG_ERROR(NULL,
                   "VIDEOIO/MSMF: Invalid usage of VIDEOWRITER_PROP_HW_DEVICE "
                   "with 'ANY' H/W acceleration. Bailout");
      return false;
    }
    if (va_device != -1) {
      CV_LOG_ERROR(NULL, "VIDEOIO/MSMF: VIDEOWRITER_PROP_HW_DEVICE is not "
                         "supported. Specify -1 (auto) value. Bailout");
      return false;
    }
  }
  ...
  if ( // Set the output media type.
      ...
      SUCCEEDED(mediaTypeOut->SetUINT32(MF_MT_INTERLACE_MODE,
                                        MFVideoInterlace_Progressive)) &&
      SUCCEEDED(MFSetAttributeSize(mediaTypeOut.Get(), MF_MT_FRAME_SIZE,
                                   videoWidth, videoHeight)) &&
      SUCCEEDED(MFSetAttributeRatio(mediaTypeOut.Get(), MF_MT_FRAME_RATE,
                                    (UINT32)(fps * 1000), 1000)) &&
      ...
      SUCCEEDED(mediaTypeIn->SetUINT32(MF_MT_INTERLACE_MODE,
                                       MFVideoInterlace_Progressive)) &&
      SUCCEEDED(mediaTypeIn->SetUINT32(MF_MT_DEFAULT_STRIDE,
                                       4 * videoWidth)) && // Assume BGR32 input
      SUCCEEDED(MFSetAttributeSize(mediaTypeIn.Get(), MF_MT_FRAME_SIZE,
                                   videoWidth, videoHeight)) &&
      SUCCEEDED(MFSetAttributeRatio(mediaTypeIn.Get(), MF_MT_FRAME_RATE,
                                    (UINT32)(fps * 1000), 1000)) &&
      SUCCEEDED(MFSetAttributeRatio(mediaTypeIn.Get(), MF_MT_PIXEL_ASPECT_RATIO,
                                    1, 1)) &&
    ...
    MultiByteToWideChar(CP_ACP, 0, filename.c_str(), -1, unicodeFileName.data(),
                        (int)filename.length() + 1);
    HRESULT hr = MFCreateSinkWriterFromURL(unicodeFileName.data(), NULL,
                                           spAttr.Get(), &sinkWriter);
    if (SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(sinkWriter->AddStream(mediaTypeOut.Get(), &streamIndex)) &&
          SUCCEEDED(sinkWriter->SetInputMediaType(streamIndex,
                                                  mediaTypeIn.Get(), NULL)) &&
          SUCCEEDED(sinkWriter->BeginWriting())) {
        initiated = true;
        rtStart = 0;
        MFFrameRateToAverageTimePerFrame((UINT32)(fps * 1000), 1000,
                                         &rtDuration);
        ...
        if (va_type != VIDEO_ACCELERATION_NONE &&
          ...
          if (va_type != actual_va_type) {
            CV_LOG_ERROR(
                NULL, "VIDEOIO/MSMF: Can't select requested video acceleration "
                      "through VIDEOWRITER_PROP_HW_ACCELERATION: "
                          << va_type << " (actual is " << actual_va_type
                          << "). Bailout");
            close();
            return false;
...
void CvVideoWriter_MSMF::write(cv::InputArray img) {
  ...
  if (SUCCEEDED(MFCreateSample(&sample)) &&
    ...
    cv::cvtColor(img.getMat(),
                 cv::Mat(videoHeight, videoWidth, CV_8UC4, pData, cbWidth),
                 img.channels() > 1 ? cv::COLOR_BGR2BGRA : cv::COLOR_GRAY2BGRA);
    buffer->Unlock();
...
cv::cvCreateVideoWriter_MSMF(const std::string &filename, int fourcc,
...
cv_capture_open_with_params(const char *filename, int camera_index, int *params,
                            unsigned n_params, CV_OUT CvPluginCapture *handle) {
  if (!handle)
...
static CvResult CV_API_CALL cv_capture_open_buffer(
    void *opaque, long long (*read)(void *opaque, char *buffer, long long size),
    long long (*seek)(void *opaque, long long offset, int way), int *params,
    unsigned n_params, CV_OUT CvPluginCapture *handle) {
  if (!handle)
    return CV_ERROR_FAIL;
  ...
  *handle = NULL;
  ...
  try {
    ...
    bool res =
        cap->open(std::string(),
                  makePtr<PluginStreamReader>(opaque, read, seek), &parameters);
    if (res) {
      *handle = (CvPluginCapture)cap;
      return CV_ERROR_OK;
    }
...
static CvResult CV_API_CALL cv_capture_open(const char *filename,
                                            int camera_index,
                                            CV_OUT CvPluginCapture *handle) {
  return cv_capture_open_with_params(filename, camera_index, NULL, 0, handle);
}

static CvResult CV_API_CALL cv_capture_release(CvPluginCapture handle) {
  if (!handle)
    return CV_ERROR_FAIL;
...
static CvResult CV_API_CALL cv_capture_get_prop(CvPluginCapture handle,
                                                int prop, CV_OUT double *val) {
  if (!handle)
    return CV_ERROR_FAIL;
  if (!val)
    return CV_ERROR_FAIL;
...
static CvResult CV_API_CALL cv_capture_set_prop(CvPluginCapture handle,
                                                int prop, double val) {
  if (!handle)
    return CV_ERROR_FAIL;
  try {
...
static CvResult CV_API_CALL cv_capture_grab(CvPluginCapture handle) {
  if (!handle)
    return CV_ERROR_FAIL;
  try {
    CaptureT *instance = (CaptureT *)handle;
...
static CvResult CV_API_CALL
cv_capture_retrieve(CvPluginCapture handle, int stream_idx,
                    cv_videoio_capture_retrieve_cb_t callback, void *userdata) {
  if (!handle)
    return CV_ERROR_FAIL;
  try {
    CaptureT *instance = (CaptureT *)handle;
...
#ifndef NEW_PLUGIN
      return callback(stream_idx, img.data, (int)img.step, img.cols, img.rows,
                      img.channels(), userdata);
#else
      return callback(stream_idx, img.data, (int)img.step, img.cols, img.rows,
                      img.type(), userdata);
#endif
...
static CvResult CV_API_CALL cv_writer_open_with_params(
    const char *filename, int fourcc, double fps, int width, int height,
    int *params, unsigned n_params, CV_OUT CvPluginWriter *handle) {
  WriterT *wrt = 0;
  try {
...
static CvResult CV_API_CALL cv_writer_open(const char *filename, int fourcc,
                                           double fps, int width, int height,
                                           int isColor,
                                           CV_OUT CvPluginWriter *handle) {
  int params[2] = {VIDEOWRITER_PROP_IS_COLOR, isColor};
  return cv_writer_open_with_params(filename, fourcc, fps, width, height,
                                    params, 1, handle);
}

static CvResult CV_API_CALL cv_writer_release(CvPluginWriter handle) {
  if (!handle)
    return CV_ERROR_FAIL;
...
static CvResult CV_API_CALL cv_writer_get_prop(CvPluginWriter handle, int prop,
                                               CV_OUT double *val) {
  if (!handle)
    return CV_ERROR_FAIL;
  if (!val)
...
static CvResult CV_API_CALL cv_writer_set_prop(CvPluginWriter /*handle*/,
                                               int /*prop*/, double /*val*/) {
  return CV_ERROR_FAIL;
}

static CvResult CV_API_CALL cv_writer_write(CvPluginWriter handle,
...
opencv_videoio_plugin_init_v0(int requested_abi_version,
  ...
  if (requested_abi_version == ABI_VERSION &&
...
static const OpenCV_VideoIO_Capture_Plugin_API capture_plugin_api = {
    {sizeof(OpenCV_VideoIO_Capture_Plugin_API), CAPTURE_ABI_VERSION,
     CAPTURE_API_VERSION, CV_VERSION_MAJOR, CV_VERSION_MINOR,
     CV_VERSION_REVISION, CV_VERSION_STATUS,
     "Microsoft Media Foundation OpenCV Video I/O plugin"},
    {
        /*  1*/ cv::CAP_MSMF,
        /*  2*/ cv::cv_capture_open,
...
opencv_videoio_capture_plugin_init_v1(int requested_abi_version,
                                      int requested_api_version,
                                      void * /*reserved=NULL*/) CV_NOEXCEPT {
  if (requested_abi_version == CAPTURE_ABI_VERSION &&
      requested_api_version <= CAPTURE_API_VERSION)
    return &capture_plugin_api;
  return NULL;
}

static const OpenCV_VideoIO_Writer_Plugin_API writer_plugin_api = {
    {sizeof(OpenCV_VideoIO_Writer_Plugin_API), WRITER_ABI_VERSION,
     WRITER_API_VERSION, CV_VERSION_MAJOR, CV_VERSION_MINOR,
     CV_VERSION_REVISION, CV_VERSION_STATUS,
     "Microsoft Media Foundation OpenCV Video I/O plugin"},
    {/*  1*/ cv::CAP_MSMF,
     /*  2*/ cv::cv_writer_open,
...
const OpenCV_VideoIO_Writer_Plugin_API *
opencv_videoio_writer_plugin_init_v1(int requested_abi_version,
                                     int requested_api_version,
                                     void * /*reserved=NULL*/) CV_NOEXCEPT {
  if (requested_abi_version == WRITER_ABI_VERSION &&
      requested_api_version <= WRITER_API_VERSION)
