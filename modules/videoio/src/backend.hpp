...
public:
    ...
    virtual Ptr<IVideoCapture> createCapture(const std::vector<uchar> &buffer, const VideoCaptureParameters& params) const = 0;
...
typedef Ptr<IVideoCapture> (*FN_createCaptureBuffer)(const std::vector<uchar> & buffer);
...
typedef Ptr<IVideoCapture> (*FN_createCaptureBufferWithParams)(const std::vector<uchar> & buffer, const VideoCaptureParameters& params);
