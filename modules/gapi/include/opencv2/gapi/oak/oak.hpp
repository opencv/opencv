...
// TODO: extend according to dai library
struct ImageManipConfig
{
    enum class ColorFormat : int { BGR, NV12 };
...
G_API_OP(GImageManip, <GFrame(GFrame, const ImageManipConfig&)>, "org.opencv.oak.image_manip") {
    static GFrameDesc outMeta(const GFrameDesc&, const ImageManipConfig&) {
        return cv::empty_gframe_desc();
    }
};
...
GAPI_EXPORTS GFrame imageManip(const GFrame& in, const ImageManipConfig& = {});
