...
struct DataQueue {
    ...
    explicit DataQueue(std::size_t capacity) {
        ...
        auto ptr = std::make_shared<SyncQueue>();
        if (capacity != 0) {
...
std::vector<Q*> input_queues(      ade::Graph &g,
                                                const ade::NodeHandle &obj) {
    ade::TypedGraph<DataQueue> qgr(g);
    std::vector<Q*> result;
...
// messages.  GStreamerExecutable can be implemented to run every
...
// this, once an Island gets Stop on an input, it starts reading all
// other input queues until it reaches Stop messages there as well.
// Only then the thread is terminated so in theory queues are left
// free'd.
//
...
class cv::gimpl::GTBBStreamingExecutor::Synchronizer final {
...
cv::gimpl::GTBBStreamingExecutor::GTBBStreamingExecutor(std::unique_ptr<ade::Graph> &&g_model,
                                                        const GCompileArgs &comp_args)
    : GStreamingExecutor(std::move(g_model), comp_args)
    , m_desync(GModel::Graph(*m_orig_graph).metadata().contains<Desynchronized>()) {
...
void cv::gimpl::GTBBStreamingExecutor::setSource(GRunArgs &&ins) {
...
bool cv::gimpl::GTBBStreamingExecutor::pull(cv::GRunArgsP &&outs) {
    ...
    return GStreamingExecutor::pull(std::move(outs));
}


bool cv::gimpl::GTBBStreamingExecutor::pull(cv::GOptRunArgsP &&outs) {
    // This pull() can be called in both cases: if there are
...
std::tuple<bool, cv::util::variant<cv::GRunArgs, cv::GOptRunArgs>> cv::gimpl::GTBBStreamingExecutor::pull() {
...
void cv::gimpl::GTBBStreamingExecutor::start_impl() {
...
bool cv::gimpl::GTBBStreamingExecutor::pull_impl(cv::GRunArgs& this_result) {
    Cmd cmd;
    m_out_queue.pop(cmd);
    switch (cmd.index()) {
        case Cmd::index_of<Stop>():
        ...
        case Cmd::index_of<Result>(): {
            ...
            this_result = cv::util::get<Result>(cmd).args;
            return true;
        }
        case Cmd::index_of<Exception>(): {
            std::rethrow_exception(cv::util::get<Exception>(cmd).eptr);
...
bool cv::gimpl::GTBBStreamingExecutor::try_pull_impl(cv::GRunArgs& this_result) {
    Cmd cmd;
...
void cv::gimpl::GTBBStreamingExecutor::stop_impl() {
...
void cv::gimpl::GTBBStreamingExecutor::wait_shutdown() {
