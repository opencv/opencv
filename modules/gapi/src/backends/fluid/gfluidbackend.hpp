...
struct FluidMapper
{
    FluidMapper(double ratio) : m_ratio(ratio) {}
    virtual ~FluidMapper() = default;
    virtual int firstWindow(int outCoord) const = 0;
    virtual int nextWindow(int outCoord) const = 0;
...
class GFluidExecutable final: public GIslandExecutable
{
    ...
    void inferRois(std::vector<int> &readStarts, std::vector<gapi::own::Rect> &rois, const std::vector<gapi::own::Rect> &out_rois);
