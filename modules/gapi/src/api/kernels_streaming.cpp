...
cv::GMat cv::gapi::streaming::desync(const cv::GMat &g, const bool drop_last) {
    ...
    // object types)
    return cv::gapi::copy(detail::desync(g, drop_last));
...
cv::GFrame cv::gapi::streaming::desync(const cv::GFrame &f, const bool drop_last) {
    return cv::gapi::copy(detail::desync(f, drop_last));
}
