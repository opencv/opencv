...
cv::GMetaArg cv::descr_of(const cv::GRunArg &arg)
{
    switch (arg.index())
    {
        ...
        case GRunArg::index_of<cv::gapi::own::RMat>():
            return cv::GMetaArg(cv::util::get<cv::gapi::own::RMat>(arg).desc());
...
bool cv::can_describe(const GMetaArg& meta, const GRunArg& arg)
{
    ...
    case GRunArg::index_of<cv::gapi::own::RMat>():   return util::holds_alternative<GMatDesc>(meta) &&
                                                            util::get<GMatDesc>(meta).canDescribe(cv::util::get<cv::gapi::own::RMat>(arg));
    default: util::throw_error(std::logic_error("Unsupported GRunArg type"));
    }
}
...
const void* cv::gimpl::proto::ptr(const GRunArgP &arg)
{
    ...
    case GRunArgP::index_of<cv::gapi::own::RMat*>():
        return static_cast<const void*>(cv::util::get<cv::gapi::own::RMat*>(arg));
