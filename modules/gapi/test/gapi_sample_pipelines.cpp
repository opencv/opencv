...
{
    cv::GMat in1, in2;
    auto out = cv::gapi::copy(in1) + cv::gapi::copy(in2);
    cv::GComputation comp(cv::GIn(in1, in2), cv::GOut(out));

    cv::Mat in_mat1 = cv::Mat::eye(32, 32, CV_8UC1);
    cv::Mat in_mat2 = cv::Mat::eye(32, 32, CV_8UC1);
    cv::Scalar ref_out = cv::sum(in_mat1 + in_mat2);

    cv::Mat ref_mat = in_mat1 + in_mat2;

    cv::Mat gapi_out;
    for (int i = 0; i < 10; ++i) {
        comp.apply(cv::gin(in_mat1, in_mat2), cv::gout(gapi_out));
        EXPECT_EQ(0, cv::norm(gapi_out, ref_mat));
    }
}

using namespace cv::gapi;

TEST(GAPI_ThreadPool, SingleTaskMultipleThreads)
{
...
TEST(GAPI_ThreadPool, WaitForTheCompletion)
{
    own::ThreadPool tp(4u);
    bool done = false;
...
TEST(GAPI_ThreadPool, MultipleWait)
{
...
TEST(GAPI_ThreadPool, MultipleTasks)
{
    own::ThreadPool tp(4u);
...
TEST(GAPI_ThreadPool, ParallelTasks)
{
    own::ThreadPool tp(4u);
    std::atomic<uint32_t> counter{0u};
...
TEST(GAPI_ThreadPool, InnerScheduleAfterWait)
{
    own::ThreadPool tp{4};
    bool done = false;
...
TEST(GAPI_ThreadPool, ParallelBenchmark)
{
    own::ThreadPool tp(4u);
